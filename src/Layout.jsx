...
import {
  Trophy, Dumbbell, Crown, MessageCircle, Users, Bell,
  Building2, Home, Flame, Award, MoreVertical, Gift,
  BarChart3, ClipboardList, CalendarDays, UserCheck,
...
/**
 * Given the previous and current tab indices, decide which direction to slide.
 *   going right → new tab is to the RIGHT  → slide from right ('right')
 *   going left  → new tab is to the LEFT   → slide from left  ('left')
 *   no tab (deep link / push) → slide up from below ('up')
 */
function resolveDirection(prevIndex, nextIndex) {
  if (prevIndex === -1 || nextIndex === -1) return 'up';
  if (nextIndex > prevIndex) return 'left';
  if (nextIndex < prevIndex) return 'right';
  return 'up';
}
...
function LayoutInner({
  ...
  const [transitionKey, setTransitionKey]   = useState(currentPageName);

  useEffect(() => {
    const nextIndex = navItems.findIndex(item => item.page === currentPageName);
    const dir = resolveDirection(prevTabIndexRef.current, nextIndex);
    setTransitionDir(dir);
    setTransitionKey(currentPageName + location.pathname + location.search);
  ...
  const getTabLink = item =>
    tabHistoryRef.current[item.page] || createPageUrl(item.page) + (item.params || '');

  const handleTabClick = (item, e) => {
    window.scrollTo({ top: 0, behavior: 'smooth' });
    if (currentPageName === item.page) {
      e.preventDefault();
      if (item.page === 'Home') window.dispatchEvent(new Event('homeButtonClicked'));
    }
  };
              ...
              return (
                ...
                >
                  ...
                  <div className="relative z-10">
                    <item.icon
                      className={`w-6 h-6 transition-colors duration-200 ${isActive ? 'text-blue-400' : ''}`}
                  ...
                  <span className={`text-[10px] font-semibold leading-none z-10 transition-colors duration-200 ${isActive ? 'text-blue-400' : ''}`}>
                    {item.name}
                  </span>
                </Link>
              );
      ...
      <main
        ...
        style={
          hideNavigation
            ? {}
            : {
                paddingBottom: 'calc(4.9375rem + env(safe-area-inset-bottom))',
                paddingTop: isCoachUser ? '3rem' : 0,
              }
        }
      >
        <ErrorBoundary>
          {/*
           * Pass `pageKey` + `direction` so PageTransition can run the right
           * animation.  The key is a combo of pageName + path so sub-page
...
export default function Layout({ children, currentPageName }) {
  ...
  const { data: notifications = [] } = useQuery({
    ...
    queryFn: () =>
      base44.entities.Notification.filter({ user_id: currentUser.id, read: false }),
  ...
  const { data: gymMemberships = [] } = useQuery({
    ...
    queryFn: () =>
      base44.entities.GymMembership.filter({ user_id: currentUser?.id, status: 'active' }),
    enabled: !!currentUser && currentUser.id !== 'guest',
    staleTime: 5 * 60 * 1000,
    gcTime: 15 * 60 * 1000,
  });
  ...
  const hideNavigation =
    ...
    (currentPageName === 'GymOwnerDashboard' && !isCoachUser);
