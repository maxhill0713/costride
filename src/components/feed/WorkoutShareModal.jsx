...
async function drawStatsCard(post, gymName, streakNum = 0) {
  ...
  // ── Inline stat row — centred, bigger labels ──
  const valFont = '900 68px -apple-system,sans-serif';
  const lblFont = '700 30px -apple-system,sans-serif'; // bigger label
  const statItems = [
  ...
  // ── Title row with streak badge — both on same line, row centred ──
  const titleText = post.workout_name || 'Workout';

  // Measure title
  let titleFontSize = 108;
  const maxTitleW = W - PAD * 2 - (streakNum > 0 ? 280 : 0); // leave room for badge
  ctx.font = `900 ${titleFontSize}px -apple-system,sans-serif`;
  while (ctx.measureText(titleText).width > maxTitleW && titleFontSize > 56) {
    titleFontSize -= 4;
    ctx.font = `900 ${titleFontSize}px -apple-system,sans-serif`;
  }
  const titleTextW = ctx.measureText(titleText).width;

  // Measure streak badge width
  let badgeTotalW = 0;
  const badgeGap = 28; // gap between title and badge
  const iconSz = 64, pillPadH = 22, pillPadV = 16;
  const numFontSz = 48;
  if (streakNum > 0) {
    ctx.font = `900 ${numFontSz}px -apple-system,sans-serif`;
    const numW = ctx.measureText(String(streakNum)).width;
    badgeTotalW = badgeGap + pillPadH + iconSz + 12 + numW + pillPadH;
  }

  // Total row width, start X for centring
  const rowTotalW = titleTextW + badgeTotalW;
  ...
  ctx.fillText(titleText, rowStartX, titleY);
  ctx.shadowBlur = 0;

  // Draw streak badge to the right of title, vertically centred on title
  if (streakNum > 0) {
    const streakImg = await loadImage(STREAK_ICON_URL);
    const pillPH = iconSz + pillPadV * 2;
    const pillX = rowStartX + titleTextW + badgeGap;
    const pillPY = titleY - titleFontSize * 0.72 - pillPH / 2 + titleFontSize * 0.12;

    drawGlassPill(ctx, pillX, pillPY, pillPadH + iconSz + 12 + ctx.measureText(String(streakNum)).width + pillPadH, pillPH, pillPH / 2);
    ...
    ctx.fillText(String(streakNum), pillX + pillPadH + iconSz + 12, pillPY + pillPH / 2 + 18);
    ctx.shadowBlur = 0;
  }
    ...
    <div style={{ width: '100%', aspectRatio: '9/16', position: 'relative', overflow: 'hidden', borderRadius: 16, background: '#0a0d16', fontFamily: "'SF Pro Display',-apple-system,sans-serif" }}>
      ...
      <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: '0 12px 18px', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, marginBottom: 7, width: '100%' }}>
          <div style={{ color: 'white', fontSize: 20, fontWeight: 900, letterSpacing: '-0.04em', lineHeight: 1, textShadow: '0 2px 16px rgba(0,0,0,0.7)' }}>
            {post.workout_name || 'Workout'}
          </div>
          {streakNum > 0 && (
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 2, background: 'rgba(255,255,255,0.12)', border: '1px solid rgba(255,255,255,0.22)', borderRadius: 20, padding: '2px 6px 2px 2px', flexShrink: 0, backdropFilter: 'blur(6px)', WebkitBackdropFilter: 'blur(6px)', boxShadow: 'inset 0 1px 0 rgba(255,255,255,0.14)' }}>
              {!iconError ? (
                <img
                  src={STREAK_ICON_URL}
                  alt=""
        ...
        {/* Inline stat row — value + label stacked, vertical rule separator */}
        ...
        {/* Footer — gym · date, bigger than before */}
