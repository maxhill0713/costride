...
// ─── STATS CARD ──────────────────────────────────────────────────────────────
// Summary card: photo bg, centred branding top, large centred title bottom,
// streak badge, then clean inline stat row (no boxes — value + label stacked,
...
async function drawStatsCard(post, gymName, streakNum = 0) {
  ...
  // Top centre branding
  await drawCentredBrand(ctx, W, 136, 60, 54);
  ...
  // ── Inline stat row — no boxes, centred text + vertical rule ──
  const valFont = '900 68px -apple-system,sans-serif';
  const lblFont = '600 24px -apple-system,sans-serif';
  const statItems = [
    { v: String(exercises.length || '—'), l: 'EXERCISES' },
    { v: post.workout_duration || '—',    l: 'DURATION'  },
  ];

  // Measure to find total width for centring
  ctx.font = valFont;
  ...
  statItems.forEach((s, i) => {
    ...
    ctx.fillText(s.l, cx, statRowY + 26);
    curX += itemW[i];
    if (i < statItems.length - 1) {
      curX += sideGap;
      ctx.beginPath();
      ctx.moveTo(curX + divW / 2, statRowY - 44);
      ctx.lineTo(curX + divW / 2, statRowY + 32);
      ctx.strokeStyle = 'rgba(255,255,255,0.18)'; ctx.lineWidth = divW;
      ctx.stroke();
      curX += divW + sideGap;
    }
  });
  ...
  const dividerY = statRowY - 44 - 40;
  ctx.strokeStyle = 'rgba(255,255,255,0.14)'; ctx.lineWidth = 2;
  ctx.beginPath(); ctx.moveTo(PAD, dividerY); ctx.lineTo(W - PAD, dividerY); ctx.stroke();

  // Streak badge — centred above divider
  if (streakNum > 0) {
    const streakImg = await loadImage(STREAK_ICON_URL);
    const iconSz = 60, pillPadH = 22, pillPadV = 14;
    ctx.font = '900 44px -apple-system,sans-serif';
    const numW = ctx.measureText(String(streakNum)).width;
    const iconGap = 10;
    const pillTotalW = pillPadH + iconSz + iconGap + numW + pillPadH;
    const pillPH = iconSz + pillPadV * 2;
    const pillX = (W - pillTotalW) / 2;
    const pillPY = dividerY - 28 - pillPH;
    drawGlassPill(ctx, pillX, pillPY, pillTotalW, pillPH, pillPH / 2);
    if (streakImg) ctx.drawImage(streakImg, pillX + pillPadH, pillPY + pillPadV, iconSz, iconSz);
    ctx.font = '900 44px -apple-system,sans-serif';
    ctx.fillStyle = 'white'; ctx.textAlign = 'left';
    ctx.shadowColor = 'rgba(0,0,0,0.6)'; ctx.shadowBlur = 6;
    ctx.fillText(String(streakNum), pillX + pillPadH + iconSz + iconGap, pillPY + pillPH / 2 + 16);
    ctx.shadowBlur = 0;
  }

  // Workout title — large, bold, CENTRED
  ...
  const titleY = dividerY - (streakNum > 0 ? 200 : 60);
  ctx.fillStyle = 'white'; ctx.textAlign = 'center';
...
async function drawBreakdownCard(post, gymName) {
  ...
  ctx.letterSpacing = '0.05em'; ctx.fillText(topLine, W / 2, 165); ctx.letterSpacing = '0';
...
function StatsPreview({ post, gymName, streakNum = 0 }) {
  ...
  return (
    <div style={{ width: '100%', aspectRatio: '9/16', position: 'relative', overflow: 'hidden', borderRadius: 16, background: '#0a0d16', fontFamily: "'SF Pro Display',-apple-system,sans-serif" }}>
      ...
      {/* Top centre branding */}
      <div style={{ position: 'absolute', top: 0, left: 0, right: 0, padding: '10px 11px 0', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 4 }}>
      ...
      <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: '0 12px 9px', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        ...
        {/* Title — centred */}
        <div style={{ color: 'white', fontSize: 21, fontWeight: 900, letterSpacing: '-0.04em', lineHeight: 1, textShadow: '0 2px 16px rgba(0,0,0,0.7)', textAlign: 'center', marginBottom: 5 }}>
          {post.workout_name || 'Workout'}
        </div>

        {/* Streak badge — centred */}
        {streakNum > 0 && (
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 3, background: 'rgba(255,255,255,0.10)', border: '1px solid rgba(255,255,255,0.20)', borderRadius: 24, padding: '2px 7px 2px 3px', marginBottom: 7, backdropFilter: 'blur(6px)', WebkitBackdropFilter: 'blur(6px)', boxShadow: 'inset 0 1px 0 rgba(255,255,255,0.12)' }}>
        ...
        {/* Inline stat row — no boxes, value + label stacked, vertical rule separator */}
          ...
          {[
            ...
            <React.Fragment key={label}>
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '0 16px' }}>
                <div style={{ color: 'white', fontSize: 17, fontWeight: 900, lineHeight: 1, marginBottom: 3 }}>{value}</div>
                <div style={{ color: 'rgba(255,255,255,0.40)', fontSize: 5.5, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.09em' }}>{label}</div>
              </div>
              {i < arr.length - 1 && (
        ...
        {/* Footer */}
        <div style={{ color: 'rgba(255,255,255,0.30)', fontSize: 5.5, fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', textAlign: 'center' }}>
          {footerLine}
        </div>
      </div>
    </div>
