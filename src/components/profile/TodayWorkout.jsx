...
export default function TodayWorkout({ currentUser }) {
  ...
  return (
    <Card className="bg-slate-800/60 border border-slate-600/40 p-6 rounded-2xl">
      ...
      {/* Workout Title */}
      <div className="mb-5">
        <h2 className="text-2xl font-black bg-gradient-to-r from-indigo-400 to-purple-400 bg-clip-text text-transparent mb-1">
          {todayWorkout.name}
        </h2>
      ...
      {todayWorkout.exercises && todayWorkout.exercises.length > 0 ? (
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-3 gap-2 mb-2">
            <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Exercise</div>
            <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Sets x Reps</div>
            <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Weight</div>
          </div>

          {/* Exercise Rows */}
          {todayWorkout.exercises.map((exercise, index) => (
            <div key={index} className="grid grid-cols-3 gap-2 p-3 bg-slate-700/50 rounded-lg border border-slate-600/30">
        ...
        <div className="p-4 bg-slate-700/50 rounded-lg border border-slate-600/30 text-center">
          <p className="text-slate-400 text-sm">Rest day - No exercises scheduled</p>
        </div>
      )}
    </Card>
  );
