      ...
      {isExpanded && todayWorkout.exercises && todayWorkout.exercises.length > 0 ? (
        <div className="space-y-2">
          ...
          {todayWorkout.exercises.map((exercise, index) => (
            <div key={index} className={`p-3 bg-white/5 backdrop-blur-md rounded-xl border border-white/10 shadow-lg shadow-black/10 ${editingIndex === index ? 'block' : 'grid grid-cols-[1fr_auto_auto_auto] gap-2 items-center'} hover:border-white/20 transition-all w-[105%]`}>
              {editingIndex === index ? (
