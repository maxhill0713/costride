...
import {
  Plus, Target, CheckCircle, ChevronRight, ChevronDown,
...
const TIME_FILTERS = [{ id: 'week', label: 'Week' }, { id: 'month', label: 'Month' }, { id: 'all', label: 'All' }];
const MEDALS = [
...
function CommunityLiftCard({ currentUser }) {
  ...
  const allTimeBest  = useMemo(() => allSets.filter(s => s.user_id === currentUser?.id && (activeLift === 'all' ? !!matchLift(s.exercise_name || '') : matchLift(s.exercise_name || '') === activeLift)).reduce((b, s) => Math.max(b, s.weight || 0), 0), [allSets, currentUser?.id, activeLift]);
...
function GoalsList({ currentUser, showAddGoal, setShowAddGoal }) {
...
export default function TargetsTab({ currentUser }) {
