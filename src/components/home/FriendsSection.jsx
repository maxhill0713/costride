...
function FriendsSection({
  ...
  return (
    <>
      {/*
        Both panels are rendered inside a single AnimatePresence wrapper.
        The wrapper mounts/unmounts as a unit (isOpen), so the slide-down/up
        animation fires only when opening from home or closing to home.
        Switching between the two panels just flips CSS visibility — no
        mount/unmount, no Framer Motion, zero animation.
      */}
      <AnimatePresence>
        {isOpen && (
          <>
            ...
            {/* Single sliding container — never unmounts while either modal is open */}
            <motion.div
              key="friends-sheet"
            ...
            >
              <div
                className="w-11/12 max-w-2xl h-[50vh] relative"
                style={{ marginTop: 'calc(1rem + env(safe-area-inset-top))' }}
              >

                {/* ── Friends panel ── */}
                <div
                  className="absolute inset-0 flex flex-col bg-slate-900/60 backdrop-blur-md border border-slate-700/20 rounded-3xl shadow-2xl shadow-black/20 text-white overflow-hidden"
                  style={{ display: showFriendsModal ? 'flex' : 'none' }}
                >
                  ...
                  <div className="flex-1 overflow-y-auto p-4 space-y-2">
                    ...
                    {sentFriendRequests.filter((req) =>
                      ...
                      const parsedDate = rawDate && !rawDate.endsWith('Z') && !rawDate.match(/[+-]\d{2}:\d{2}$/)
                        ? new Date(rawDate + 'Z')
                        : new Date(rawDate);
                      const sentMs = Date.now() - parsedDate.getTime();
                      const sentMins = Math.floor(sentMs / (1000 * 60));
                    ...
                    {friends.length === 0 && friendRequests.length === 0 && sentFriendRequests.length === 0
                      ...
                      : friendsWithActivity.filter((friend) => {
                          ...
                          const displayName = userData?.display_name || userData?.full_name || friend.friend_name || 'User';
                          return displayName.toLowerCase().includes(friendsListSearchQuery.toLowerCase());
                        }).map((friend) => {
                          const userData = userDataMap[friend.friend_id];
                          const name = userData?.display_name || userData?.full_name || friend.friend_name || 'User';
                ...
                <div
                  className="absolute inset-0 flex flex-col bg-slate-900/60 backdrop-blur-md border border-slate-700/20 rounded-3xl shadow-2xl shadow-black/20 text-white overflow-hidden"
                  style={{ display: showAddFriendModal ? 'flex' : 'none' }}
                >
                  <div className="px-3 py-1 flex items-center gap-1">
                    <div className="relative flex-1">
