        ...
        <>
          ...
          <motion.div initial={{scale:0.82,opacity:0}} animate={{scale:1,opacity:1}} exit={{scale:0.82,opacity:0}}
            ...
            <div style={{fontSize:11,color:'rgba(255,255,255,0.22)',padding:'8px 12px',background:'rgba(255,255,255,0.04)',borderRadius:9,border:'1px solid rgba(255,255,255,0.06)'}}>
              Valid for {gymClass?.name||'this class'} only
            </div>
          </motion.div>
        </>
      )}
...
export default function ClassDetailModal({
  ...
  const c    = CFG[tk];
  const img  = gymClass.image_url||IMGS[tk]||IMGS.default;
  ...
  const barColor = full?'linear-gradient(90deg,#dc2626,#f87171)':pct>65?'linear-gradient(90deg,#d97706,#fbbf24)':`linear-gradient(90deg,${c.color}88,${c.color})`;
  ...
  const ratCounts=[5,4,3,2,1].map(s=>({s,n:REVIEWS.filter(r=>r.rating===s).length,p:Math.round(REVIEWS.filter(r=>r.rating===s).length/REVIEWS.length*100)}));
  ...
  const MOCK_ATT=[{id:'a1',name:'Alex M.',color:'#3b82f6'},{id:'a2',name:'Sara K.',color:'#8b5cf6'},{id:'a3',name:'Tom B.',color:'#10b981'},{id:'a4',name:'Jen L.',color:'#f59e0b'}];
  const att=[...friendsBooked,...MOCK_ATT].slice(0,5);
  ...
  return(
    <>
                      ...
                      {(gymClass.instructor||gymClass.coach_name)&&(
                        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between'}}>
                          <div style={{display:'flex',alignItems:'center',gap:9}}>
                            ...
                            <div>
                              <div style={{fontSize:12.5,fontWeight:800,color:'#fff',lineHeight:1}}>{gymClass.instructor||gymClass.coach_name}</div>
                              <div style={{fontSize:10,color:'rgba(255,255,255,0.38)',fontWeight:600,marginTop:2}}>Lead Instructor</div>
                            </div>
                          </div>
                          <div style={{display:'flex',alignItems:'center',gap:5}}>
                            <div style={{display:'flex',gap:1}}>{[1,2,3,4,5].map(s=><Star key={s} style={{width:10,height:10,fill:s<=Math.round(parseFloat(avg))?'#fbbf24':'rgba(255,255,255,0.15)',color:s<=Math.round(parseFloat(avg))?'#fbbf24':'rgba(255,255,255,0.15)'}}/>)}</div>
                            <span style={{fontSize:12,fontWeight:800,color:'#fbbf24'}}>{avg}</span>
                            <span style={{fontSize:10,color:'rgba(255,255,255,0.3)'}}>({REVIEWS.length})</span>
                          </div>
                        </div>
                      )}
                    </motion.div>
                  ...
                  {tab==='details'&&(
                    <motion.div key="d" initial={{opacity:0,y:8}} animate={{opacity:1,y:0}} transition={{duration:0.18}}
                      ...
                      <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:10}}>
                        ...
                        <StatCard icon={Zap}    label="Level"    value={gymClass.difficulty?gymClass.difficulty.replace('_',' ').replace(/\b\w/g,l=>l.toUpperCase()):'All Levels'} color={c.color}/>
                      </div>
                      ...
                      <div style={{background:CARD_BG,border:CARD_BORDER,borderRadius:16,overflow:'hidden'}}>
                        ...
                        <div style={{padding:'6px 15px 12px'}}>
                          {[
                            {I:Zap,      l:'Intensity',       v:gymClass.difficulty?gymClass.difficulty.replace('_',' ').replace(/\b\w/g,x=>x.toUpperCase()):'All Levels'},
                            {I:Dumbbell, l:'Equipment',       v:gymClass.equipment||'Standard gym equipment'},
                            {I:TrendingUp,l:'Est. Calories',  v:gymClass.calories?`~${gymClass.calories} kcal`:gymClass.duration_minutes?`~${Math.round(gymClass.duration_minutes*7)} kcal`:'Varies'},
                            {I:Award,    l:'Class Type',      v:c.label},
                      ...
                      <div style={{display:'flex',alignItems:'flex-start',gap:10,padding:'12px 14px',background:'rgba(251,191,36,0.05)',border:'1px solid rgba(251,191,36,0.15)',borderRadius:14}}>
                        ...
                        <div>
                          ...
                          <div style={{fontSize:11.5,color:'rgba(255,255,255,0.42)',lineHeight:1.55}}>Cancel up to 4 hours before class starts for a full refund. Late cancellations may incur a fee.</div>
                        </div>
                      </div>
                              ...
                              <div key={i} style={{display:'flex',alignItems:'center',gap:12,
                                ...
                                <div style={{flex:1}}>
                                  ...
                                  <div style={{display:'flex',gap:8,marginTop:3}}>
                                    {gymClass.schedule&&<span style={{fontSize:11,color:'rgba(255,255,255,0.38)'}}>{gymClass.schedule}</span>}
                  ...
                  {tab==='reviews'&&(
                    <motion.div key="r" initial={{opacity:0,y:8}} animate={{opacity:1,y:0}} transition={{duration:0.18}}
                      ...
                      <div style={{background:CARD_BG,border:CARD_BORDER,borderRadius:16,padding:'16px'}}>
                        <div style={{display:'flex',gap:16,alignItems:'center'}}>
                          <div style={{textAlign:'center',flexShrink:0}}>
                            ...
                            <div style={{fontSize:10,color:'rgba(255,255,255,0.28)',marginTop:5,fontWeight:600}}>{REVIEWS.length} reviews</div>
                          </div>
                      ...
                      <div style={{display:'flex',flexDirection:'column',gap:10}}>
                        {(moreReviews?REVIEWS:REVIEWS.slice(0,2)).map((r,i)=><ReviewCard key={i} r={r} c={c}/>)}
                      </div>
      ...
      <RateSheet open={showRate} onClose={()=>setShowRate(false)} c={c} className={gymClass?.name||'Class'}/>
    </>
  );
}
