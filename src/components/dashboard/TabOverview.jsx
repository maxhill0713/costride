...
import { Plus, Trophy, BarChart2, MessageSquarePlus, Calendar, ChevronRight, TrendingUp, Zap, Heart, MessageCircle } from 'lucide-react';
      ...
      {content && (
        <div style={{ padding: '0 14px 10px' }}>
          ...
          {post.title && content !== post.title && (
            <p style={{ fontSize: 12, color: 'var(--text2)', margin: '4px 0 0', lineHeight: 1.5 }}>
              {content}
            </p>
          )}
        </div>
...
export default function TabContent({
  ...
  const feedItems = useMemo(() => {
    switch (activeFilter) {
      case 'members':    return { posts: memberPosts,  events: [], challenges: [], polls: [] };
      case 'gym':        return { posts: gymPosts,     events: [], challenges: [], polls: [] };
      case 'challenges': return { posts: [],           events: [], challenges: activeChallenges, polls: [] };
      case 'classes':    return { posts: [],           events: upcomingEvents, challenges: [], polls: [] };
      case 'polls':      return { posts: [],           events: [], challenges: [], polls: polls };
      default:           return { posts: allPosts,     events: upcomingEvents, challenges: activeChallenges, polls: polls };
    }
  }, [activeFilter, allPosts, gymPosts, memberPosts, upcomingEvents, activeChallenges, polls]);
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1fr) clamp(260px,22%,320px)', gap: 16, height: '100%', maxWidth: '100%' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden', minHeight: 0 }}>
        ...
        {/* Feed header + filter tabs row */}
        <div style={{ flexShrink: 0, marginBottom: 0 }}>
          {/* Title row */}
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 0, padding: '0 0 0 2px' }}>
          ...
          {/* Filter tabs — underline style like screenshot */}
          <div style={{
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                {col1.map((item, i) => {
                  ...
                  if (item.type === 'poll')      return (
                    <div key={item.data.id || i} style={{ borderRadius: 12, background: 'var(--card2)', border: '1px solid rgba(139,92,246,0.15)', padding: '12px 14px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8 }}>
                        <BarChart2 style={{ width: 13, height: 13, color: '#a78bfa' }}/>
                        <span style={{ fontSize: 10, fontWeight: 700, color: '#a78bfa' }}>Poll</span>
                      </div>
                      <p style={{ fontSize: 13, fontWeight: 700, color: 'var(--text1)', margin: '0 0 8px' }}>{item.data.title}</p>
                      <div style={{ fontSize: 11, color: 'var(--text3)' }}>{item.data.voters?.length || 0} votes</div>
                    </div>
                  );
                  return null;
                })}
