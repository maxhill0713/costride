...
function SC({icon:Icon,label,value,sub,prefix="",delay=0,highlight,alertRed}){
  const c=useCountUp(typeof value==="number"?value:0,delay);
  const d=typeof value==="number"?`${prefix}${c.toLocaleString()}`:value;
  return <Card style={{padding:"18px 20px"}}>
    ...
    <div style={{fontSize:28,fontWeight:700,letterSpacing:"-0.04em",lineHeight:1,color:alertRed?T.red:highlight?T.accent:T.t1,marginBottom:5,fontVariantNumeric:"tabular-nums"}}>{d}</div>
...
function ChurnCard({m,onMsg,onSel}){
  ...
  return <div onMouseEnter={()=>sH(true)} onMouseLeave={()=>sH(false)} onClick={()=>onSel(m)}
    style={{padding:"16px 18px",background:hov?T.surfaceHov:T.surface,border:`1px solid ${T.border}`,borderLeft:`2px solid ${bc}`,borderRadius:T.r,boxShadow:T.sh,cursor:"pointer",transition:"background .12s"}}>
    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12}}>
      ...
      <div style={{textAlign:"right"}}>
        <div style={{fontSize:20,fontWeight:700,color:bc,fontVariantNumeric:"tabular-nums",lineHeight:1}}>{m.ch}%</div>
  ...
  return <div>
    ...
    {sorted.length===0
      ...
      :sorted.map((m,idx)=>{
        ...
        const bc=m.ch>=70?T.red:m.ch>=40?T.amber:T.t3;
        return <div key={m.id} onClick={()=>setPrev(isPrev?null:m)}
...
function BulkBar({selRows,members,onClear,onBulk}){
  ...
  return <div style={{borderTop:`1px solid ${T.borderEl}`,background:T.surfaceEl}}>
...
function Sidebar({members,onFilter}){
  ...
  return <div style={{display:"flex",flexDirection:"column",gap:8}}>
    ...
    {nq.length>0&&<Card style={{padding:"14px 16px"}}>
      <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:8}}><UserPlus size={11} color={T.t3}/><span style={{fontSize:12,fontWeight:600,color:T.t1}}>New members going quiet</span></div>
      <div style={{display:"flex",gap:4,marginBottom:8,flexWrap:"wrap"}}>
        {nq.map((m,i)=>(
          <div key={i} style={{display:"flex",alignItems:"center",gap:4,padding:"2px 7px 2px 3px",borderRadius:20,background:T.surfaceEl,border:`1px solid ${T.border}`}}>
            <Av m={m} size={14}/><span style={{fontSize:10,color:T.t2}}>{m.name.split(" ")[0]}</span>
    ...
    <Card style={{padding:"14px 16px"}}>
      <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:10}}><TrendingDown size={11} color={T.t3}/><span style={{fontSize:12,fontWeight:600,color:T.t1}}>Drop-off patterns</span></div>
...
function Preview({m,onClose,onMsg}){
  ...
  return <div style={{position:"fixed",top:0,right:0,bottom:0,width:310,background:T.surface,borderLeft:`1px solid ${T.border}`,zIndex:200,display:"flex",flexDirection:"column",boxShadow:"-12px 0 40px rgba(0,0,0,0.6)",animation:"panelIn .18s ease"}}>
    ...
    <div style={{flex:1,overflowY:"auto",padding:"16px 18px"}}>
      {m.ch>=40&&<div style={{padding:"12px 14px",borderRadius:T.r,marginBottom:12,background:T.surfaceEl,border:`1px solid ${T.border}`}}>
...
function Toast({member,onClose}){
  ...
  return <div style={{position:"fixed",bottom:80,right:26,width:350,background:T.surface,border:`1px solid ${T.borderEl}`,borderRadius:T.r,boxShadow:"0 4px 24px rgba(0,0,0,0.5)",zIndex:300,overflow:"hidden",animation:"toastIn .18s ease"}}>
...
export default function MembersPage(){
  ...
  const counts=useMemo(()=>({all:members.length,atRisk:members.filter(m=>m.ch>=60).length,dropping:members.filter(m=>m.pv>0&&m.v30<=m.pv*.5).length,new:members.filter(m=>m.jd<=14).length,active:members.filter(m=>m.str>=5).length,inactive:members.filter(m=>m.ds>=14).length}),[]);
    ...
    <div style={{minHeight:"100vh",background:T.bg,fontFamily:"'Geist','DM Sans','Helvetica Neue',Arial,sans-serif",color:T.t1,fontSize:13,lineHeight:1.5}}>
      ...
      <div style={{maxWidth:1320,margin:"0 auto",padding:"24px 24px 80px"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10,marginBottom:16}}>
          ...
          <SC icon={AlertTriangle} label="At Risk"         sub="60%+ churn probability"                                value={ar.length}      delay={240} alertRed={ar.length>0}/>
