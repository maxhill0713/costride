...
import { Users, Activity, AlertCircle, Flame, MessageCircle, ChevronRight, Trophy, Search } from 'lucide-react';
...
export default function TabCoachMembers({ allMemberships, checkIns, ci30, avatarMap, openModal, now }) {
  ...
  const memberLastCI = useMemo(() => {
    const map = {};
    checkIns.forEach(c => {
      if (!map[c.user_id] || new Date(c.check_in_date) > new Date(map[c.user_id]))
        map[c.user_id] = c.check_in_date;
    });
    return map;
  }, [checkIns]);
  ...
  const enriched = useMemo(() => allMemberships.map(m => {
    ...
    return { ...m, last, daysAgo, visits, trend, streak, status, spark, totalVisits, nextMilestone };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12 }}>
        ...
        <CoachKpiCard icon={AlertCircle} label="Need Attention"   value={counts.at_risk + counts.inactive}                 sub="missing 14+ days"   accentColor="#ef4444" subColor={counts.at_risk > 0 ? '#f87171' : '#34d399'}/>
        <CoachKpiCard icon={Flame}       label="Avg Streak"       value={`${avgStreak}d`}                                  sub="consecutive days"   accentColor="#f59e0b"/>
      </div>

      {/* Controls bar */}
      <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
        {/* View toggle */}
        <div style={{ display: 'flex', gap: 2, padding: '3px', background: 'rgba(255,255,255,0.02)', borderRadius: 10, border: '1px solid rgba(255,255,255,0.06)', flexShrink: 0 }}>
          {[{ id:'list', label:'👤 Members' }, { id:'leaderboard', label:'🏆 Leaderboard' }].map(v => (
            <button key={v.id} onClick={() => setViewMode(v.id)} style={{ padding: '6px 13px', borderRadius: 8, border: viewMode===v.id ? '1px solid rgba(255,255,255,0.08)' : '1px solid transparent', background: viewMode===v.id ? '#0c1a2e' : 'transparent', color: viewMode===v.id ? '#a78bfa' : '#3a5070', fontSize: 11, fontWeight: viewMode===v.id ? 700 : 500, cursor: 'pointer', whiteSpace: 'nowrap' }}>
              {v.label}
            </button>
          ))}
        </div>
          ...
          <>
            ...
            <div style={{ position: 'relative', flex: 1, minWidth: 160 }}>
              ...
              <input
                ...
                style={{ width: '100%', padding: '8px 12px 8px 30px', borderRadius: 10, background: '#0c1a2e', border: '1px solid rgba(255,255,255,0.07)', color: '#f0f4f8', fontSize: 12, outline: 'none', boxSizing: 'border-box' }}
              />
            </div>
            {/* Status filter */}
            <div style={{ display: 'flex', gap: 2, padding: '3px', background: 'rgba(255,255,255,0.02)', borderRadius: 10, border: '1px solid rgba(255,255,255,0.06)', flexShrink: 0 }}>
            ...
            <select value={sort} onChange={e => setSort(e.target.value)} style={{ padding: '7px 10px', borderRadius: 9, background: '#0c1a2e', border: '1px solid rgba(255,255,255,0.07)', color: '#94a3b8', fontSize: 11, outline: 'none', cursor: 'pointer', flexShrink: 0 }}>
              ...
              <option value="atRisk">Needs Attention</option>
              ...
              return (
                <div key={m.user_id||i} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '11px 16px', borderBottom: i < lbData.length-1 ? '1px solid rgba(255,255,255,0.04)' : 'none', background: i < 3 ? `rgba(251,191,36,${0.03 - i*0.01})` : 'transparent' }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ marginTop: 5, height: 3, borderRadius: 99, background: 'rgba(255,255,255,0.05)', overflow: 'hidden', maxWidth: 180 }}>
          ...
          {filtered.length === 0 ? (
            ...
            return (
              <div key={m.user_id || i}>
                {/* Row */}
                <div
                  style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '13px 16px', borderBottom: '1px solid rgba(255,255,255,0.04)', cursor: 'pointer', transition: 'background 0.12s', background: isExp ? 'rgba(167,139,250,0.04)' : 'transparent' }}
                  onClick={() => setExpanded(isExp ? null : (m.user_id || i))}
                  onMouseEnter={e => { if (!isExp) e.currentTarget.style.background = 'rgba(255,255,255,0.02)'; }}
                  onMouseLeave={e => { if (!isExp) e.currentTarget.style.background = isExp ? 'rgba(167,139,250,0.04)' : 'transparent'; }}>

                  {/* Avatar + status dot */}
                  <div style={{ position: 'relative', flexShrink: 0 }}>
                    <MiniAvatar name={m.user_name} src={avatarMap[m.user_id]} size={38} color="#a78bfa"/>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
                      ...
                      {m.streak >= 3 && <span style={{ fontSize: 9, color: '#f59e0b', background: 'rgba(245,158,11,0.1)', borderRadius: 4, padding: '2px 5px', flexShrink: 0 }}>🔥 {m.streak}d</span>}
                    </div>
                  ...
                  <div style={{ flexShrink: 0, width: 52 }}>
                    <div style={{ fontSize: 12, fontWeight: 700, color: '#f0f4f8', textAlign: 'right', marginBottom: 3 }}>{m.visits}<span style={{ fontSize: 9, color: '#3a5070', fontWeight: 400 }}>/mo</span></div>
                ...
                {/* Expanded detail */}
                {isExp && (
                  <div style={{ padding: '16px 16px 18px', background: 'rgba(167,139,250,0.03)', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                    {/* Stats grid */}
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 8, marginBottom: 14 }}>
                      {[
                        ...
                        { label: 'Next Milestone', value: m.nextMilestone || '—', color: '#34d399' },
                      ].map((s, si) => (
                        <div key={si} style={{ padding: '10px 12px', borderRadius: 10, background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.05)', textAlign: 'center' }}>
                          ...
                          <div style={{ fontSize: 9, color: '#3a5070', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em', marginTop: 3 }}>{s.label}</div>
                        </div>
                      ))}
                    </div>
                    ...
                    <div style={{ marginBottom: 14 }}>
                      <div style={{ fontSize: 9, fontWeight: 700, color: '#3a5070', textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 7 }}>Last 14 Days</div>
                      <div style={{ display: 'flex', gap: 4 }}>
                        {m.spark.map((v, si) => (
                          <div key={si} title={format(subDays(now,13-si),'MMM d')} style={{ flex: 1, aspectRatio: '1', borderRadius: 5, background: v > 0 ? `${sc.color}cc` : 'rgba(255,255,255,0.05)', border: `1px solid ${v > 0 ? sc.color+'40' : 'rgba(255,255,255,0.06)'}`, maxWidth: 22 }}/>
                        ))}
                      </div>
                    </div>
                    ...
                    {/* Coach note */}
                    <div>
                      <div style={{ fontSize: 9, fontWeight: 700, color: '#3a5070', textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 7, display: 'flex', alignItems: 'center', gap: 6 }}>
                        Coach Note
                        {notes[m.user_id] && <span style={{ fontSize: 9, color: '#34d399', fontWeight: 600 }}>✓ saved</span>}
                      </div>
                      <textarea
                        placeholder={`Add a private note about ${m.user_name || 'this member'}…`}
