...
function LivePreview({ draft, onOpenModal }) {
  ...
  return (
    <div style={{ position: 'sticky', top: 20, display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      <button onClick={onOpenModal} className="tcp-btn"
        style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 7, padding: '11px', borderRadius: 12, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.09)', color: BLUE_LT, fontSize: 12, fontWeight: 700, width: '100%' }}>
        <Eye style={{ width: 13, height: 13 }} /> View Full Profile
      </button>
    </div>
  );
}
...
function ProfileEntryCard({ coach, onOpen, onOpenModal }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: 360, gap: 20, fontFamily: 'Figtree,system-ui,sans-serif' }}>
      ...
      >
        ...
        <div style={{ padding: '14px 18px 18px' }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <button onClick={onOpenModal} className="tcp-btn"
              style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, padding: '10px', borderRadius: 11, background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', color: BLUE_LT, fontSize: 12, fontWeight: 700 }}>
              <Eye style={{ width: 12, height: 12 }} /> View Profile
            </button>
            <button onClick={onOpen} className="tcp-btn"
              style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, padding: '10px', borderRadius: 11, background: `linear-gradient(135deg,${BLUE},#1d4ed8)`, border: 'none', color: '#fff', fontSize: 12, fontWeight: 800, boxShadow: '0 4px 16px rgba(37,99,235,0.4)' }}>
      ...
      <p style={{ fontSize: 12, color: MUTE, textAlign: 'center', maxWidth: 300 }}>
        Click <strong style={{ color: SUB }}>Edit Profile</strong> to update your details, or <strong style={{ color: SUB }}>View Profile</strong> to see how members see you.
      </p>
    </div>
  );
}
...
function ProfileEditorOverlay({ selectedGym, currentUser, onClose, coach, draft, setDraft, dirty, setDirty, handleSave, handleDiscard, uploading, heroUploading, handleAvatarUpload, handleHeroUpload, updateMutation, createMutation, showPreviewModal, setShowPreviewModal, sectionRefs, patch }) {
  return (
  ...
  return (
    <>
      ...
      <ProfileEntryCard
        ...
        onOpenModal={() => setShowPreviewModal(true)}
      />
