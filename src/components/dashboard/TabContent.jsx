...
/* ─── EDIT EVENT MODAL — Save only active when dirty ────────────── */
        ...
        <div style={{ display: "flex", gap: 8, justifyContent: "flex-end", marginTop: 4 }}>
          ...
          <button
            ...
            style={{
              ...
              background: canSave ? C.cyan : C.brd,
              border: "none",
...
/* ══════════════════════════════════════════════════════════════
   EVENT DETAIL POPUP — button colours match community feed style
══════════════════════════════════════════════════════════════ */
    ...
    <>
      ...
      >
        <div style={{ background: C.card, border: `1px solid ${C.brd}`, borderRadius: 16, width: 440, maxWidth: "92vw", overflow: "hidden", boxShadow: "0 32px 80px rgba(0,0,0,0.8), 0 0 0 1px rgba(77,127,255,0.06)" }}>
          ...
          {/* Footer — colours now match community feed quick action buttons */}
          <div style={{ padding: "12px 20px 16px", borderTop: `1px solid ${C.brd}`, display: "flex", gap: 8, justifyContent: "flex-end" }}>
            {/* Edit Event — matches community feed "Edit Post" style: subtle hover → cyan */}
            <button
            ...
            {/* Remove Event — matches community feed "Remove Post" style: subtle hover → red */}
            <button
...
/* ══════════════════════════════════════════════════════════════
   CREATE EVENT MODAL — with Repeat Event button in footer left
══════════════════════════════════════════════════════════════ */
...
const CE_MONO = { fontVariantNumeric: 'tabular-nums' };
...
export function CreateEventModal({ open, onClose, onSave, gym, isLoading, existingEvents = [] }) {
  ...
  // When user picks an event to repeat, fill form (minus date)
  const handleRepeatSelect = (ev) => {
    setForm({
      title: ev.title || '',
      description: ev.description || '',
      event_date: '',       // date left blank for user to fill
      ...
      >
        ...
        <div style={{
            ...
            {isMobile ? (
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <button
                    ...
                    style={{
                      ...
                      background: showPreview ? CE.blueDim : 'transparent',
                      border: `1px solid ${showPreview ? CE.blueBrd : CE.brd}`,
                      cursor: 'pointer', color: showPreview ? CE.blue : CE.t3, fontSize: 11, fontWeight: 600, fontFamily: CE_FONT,
                    }}
                  >
              ...
              <>
                <div style={{ display: 'flex', alignItems: 'center', gap: 11 }}>
                  ...
                  <div style={{ fontSize: 18, fontWeight: 700, color: CE.t1, letterSpacing: '-0.02em', lineHeight: 1.2 }}>
                    Create Event
                  </div>
                </div>
                <button
          ...
          >
            {(!isMobile || !showPreview) && (
              <div className="ce-scroll" style={{
                ...
                <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: isMobile ? 14 : 12 }}>
                  <CEField label="Date & Time" required>
            ...
            {(!isMobile || showPreview) && (
              <div className="ce-scroll" style={{
                ...
                background: CE.surface, overflowY: 'auto',
                borderLeft: isMobile ? 'none' : `1px solid ${CE.brd}`,
                WebkitOverflowScrolling: 'touch',
              }}>
          ...
          <div style={{
            ...
            {/* LEFT: Repeat Event button (desktop) */}
            {!isMobile && (
              <div style={{ flex: 1 }}>
                <button
                  type="button"
                  ...
                  style={{
                    ...
                    padding: '7px 14px', borderRadius: 8,
                    background: 'transparent', border: `1px solid ${CE.brd}`,
                    color: CE.t2, fontSize: 12, fontWeight: 600,
                    cursor: 'pointer', fontFamily: CE_FONT, transition: 'all 0.15s',
                  }}
            ...
            {isMobile ? (
              <>
                <button
                  ...
                  onClick={() => setShowRepeatPicker(true)}
                  style={{
                    width: '100%', height: 44, borderRadius: 10, border: `1px solid ${CE.brd}`,
                    fontFamily: CE_FONT, fontSize: 13, fontWeight: 600,
                    background: 'transparent', color: CE.t2,
                    cursor: 'pointer',
                ...
                <button
                  ...
                  style={{
                    ...
                    fontFamily: CE_FONT, fontSize: 15, fontWeight: 700, letterSpacing: '-0.01em',
                    background: canSubmit ? CE.blue : CE.brd2,
                    color: canSubmit ? '#fff' : CE.t3,
                    cursor: canSubmit ? 'pointer' : 'default',
                    display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 8,
                    boxShadow: canSubmit ? `0 0 24px ${CE.blue}40` : 'none',
              ...
              <>
                ...
                <button
                  ...
                  style={{
                    ...
                    fontFamily: CE_FONT, fontSize: 13, fontWeight: 700, letterSpacing: '-0.01em',
                    background: canSubmit ? CE.blue : CE.brd2,
                    color: canSubmit ? '#fff' : CE.t3,
                    cursor: canSubmit ? 'pointer' : 'default',
