    ...
    <div className={`workout-card${isSelected ? ' selected' : ''}`} onClick={() => onSelect(workout)}>
      ...
      <div style={{ padding: '14px 15px' }}>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 10, flexWrap: 'wrap' }}>
          ...
          <span style={{ fontSize: 10, color: '#64748b' }}>{totalExercises} exercises</span>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 3, marginBottom: 12 }}>
          {workout.main.slice(0, 3).map((ex, i) => (
            <div key={i} style={{ fontSize: 11, color: '#94a3b8', display: 'flex', alignItems: 'center', gap: 6 }}>
              <div style={{ width: 4, height: 4, borderRadius: '50%', background: tc.color, flexShrink: 0 }}/>
          ...
          {workout.main.length > 3 && <div style={{ fontSize: 10, color: '#3a5070', paddingLeft: 10 }}>+{workout.main.length - 3} more</div>}
        </div>
...
function ExerciseRow({ ex, onChange, onDelete }) {
  const inputS = {
    padding: '5px 8px', borderRadius: 6, background: '#060c18',
    border: '1px solid rgba(255,255,255,0.07)', color: '#f0f4f8',
        ...
        <div style={{ padding: '14px 20px' }}>
            ...
            <>
              ...
              {filtered.length === 0 ? (
                <div style={{ textAlign: 'center', padding: '20px 0', color: '#3a5070' }}>
                  <p style={{ fontSize: 12, fontWeight: 600, margin: 0 }}>No {tab === 'class' ? 'classes' : 'members'} found</p>
                </div>
              ) : filtered.map((item, i) => {
                ...
                return (
                  <div key={id || i} onClick={() => toggle(id)} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '9px 10px', borderRadius: 10, background: isChosen ? 'rgba(167,139,250,0.07)' : 'rgba(255,255,255,0.02)', border: `1px solid ${isChosen ? 'rgba(167,139,250,0.25)' : 'rgba(255,255,255,0.05)'}`, cursor: 'pointer', marginBottom: 6, transition: 'all 0.1s' }}>
          ...
          {(tab === 'class' || tab === 'member') && assigned.length > 0 && (
            <button onClick={() => { openModal('assignWorkout', { workoutId: workout.id, workoutName: workout.name, assignTo: tab, ids: assigned }); onClose(); }}
              style={{ width: '100%', marginTop: 14, padding: '11px', borderRadius: 11, background: `linear-gradient(135deg,${tc.color}30,${tc.color}18)`, border: `1px solid ${tc.color}40`, color: tc.color, fontSize: 13, fontWeight: 800, cursor: 'pointer', transition: 'all 0.15s' }}>
            ...
            <>
              ...
              {filteredWorkouts.length === 0 ? (
                <div style={{ textAlign: 'center', padding: '36px 0', borderRadius: 16, background: '#0c1a2e', border: '1px solid rgba(255,255,255,0.07)' }}>
                  ...
                  <p style={{ fontSize: 12, color: '#3a5070', fontWeight: 600, margin: '0 0 12px' }}>No workouts yet</p>
                  <button onClick={handleNew} style={{ fontSize: 11, fontWeight: 700, color: '#a78bfa', background: 'rgba(167,139,250,0.1)', border: '1px solid rgba(167,139,250,0.2)', borderRadius: 8, padding: '7px 14px', cursor: 'pointer' }}>Create your first workout</button>
                </div>
              ) : (
              ...
              {selectedWO && (
                <div style={{ borderRadius: 18, background: '#0c1a2e', border: `1px solid ${(WORKOUT_TYPES[selectedWO.type] || WORKOUT_TYPES.strength).color}30`, overflow: 'hidden' }}>
                  {(() => {
                    const tc = WORKOUT_TYPES[selectedWO.type] || WORKOUT_TYPES.strength;
                    return (
                      <>
                        {/* Detail header */}
                        <div style={{ padding: '16px 18px 14px', background: `${tc.color}06`, borderBottom: `1px solid ${tc.color}18`, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                            <div style={{ width: 36, height: 36, borderRadius: 10, background: tc.bg, border: `1px solid ${tc.border}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16 }}>{tc.emoji}</div>
                            <div>
                              <div style={{ fontSize: 15, fontWeight: 900, color: '#f0f4f8' }}>{selectedWO.name}</div>
                              <div style={{ display: 'flex', gap: 8, marginTop: 3 }}>
                                <span style={{ fontSize: 10, color: tc.color, background: tc.bg, borderRadius: 4, padding: '1px 6px', fontWeight: 700 }}>{tc.label}</span>
                                {selectedWO.duration && <span style={{ fontSize: 10, color: '#64748b' }}>⏱ {selectedWO.duration}min</span>}
                                {selectedWO.difficulty && <span style={{ fontSize: 10, color: '#64748b' }}>{selectedWO.difficulty}</span>}
                              </div>
                            </div>
                          </div>
                          <div style={{ display: 'flex', gap: 7, alignItems: 'center' }}>
                            <button onClick={() => handleEdit(selectedWO)} style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '7px 12px', borderRadius: 9, background: `${tc.color}0f`, border: `1px solid ${tc.color}25`, color: tc.color, fontSize: 11, fontWeight: 700, cursor: 'pointer' }}>
                              <Edit2 style={{ width: 11, height: 11 }}/> Edit
                            </button>
                            <button onClick={() => setAssignWO(selectedWO)} style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '7px 14px', borderRadius: 9, background: `linear-gradient(135deg,${tc.color}28,${tc.color}14)`, border: `1px solid ${tc.color}40`, color: tc.color, fontSize: 11, fontWeight: 800, cursor: 'pointer' }}>
                              <Play style={{ width: 11, height: 11 }}/> Assign
                            </button>
                            <button onClick={() => setSelectedWO(null)} style={{ width: 28, height: 28, borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.09)', color: '#64748b', cursor: 'pointer' }}>
                              <X style={{ width: 12, height: 12 }}/>
                            </button>
                          </div>
                        </div>
                        ...
                        {/* Section breakdown */}
                        <div style={{ padding: '16px 18px', display: 'grid', gridTemplateColumns: '1fr 2fr 1fr', gap: 16 }}>
                          {[
