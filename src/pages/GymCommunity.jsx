...
export default function GymCommunity() {
  ...
  const rsvpMutation = useMutation({
    ...
    base44.entities.Event.update(eventId, { attendees: currentAttendees + 1 }),
  ...
  const createRewardMutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['rewards', gymId] }); }
  });
  ...
  const claimRewardMutation = useMutation({
    ...
    base44.entities.Reward.update(rewardId, { claimed_by: [...currentClaimed, userId] }),
  ...
  const createClassMutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['classes', gymId] }); }
  });
  ...
  const createCoachMutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['coaches', gymId] }); }
  });
  ...
  const deleteChallengeMutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['challenges', gymId] }); }
  });
  ...
  const deleteEventMutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['events', gymId] }); }
  });
  ...
  const createChallengeMutation = useMutation({
    mutationFn: (challengeData) => {
      const fullData = { ...challengeData, gym_id: gymId, gym_name: gym?.name };
    ...
    onError: (error) => { console.error('Challenge creation failed:', error); }
  });
  ...
  const meetsRequirement = (requirement) => {
    ...
    switch (requirement) {
      case 'first_visit': return userCheckIns.length >= 1;
      case 'visits_3': return userCheckIns.length >= 3;
      case 'visits_5': return userCheckIns.length >= 5;
      case 'visits_10': case 'check_ins_10': return userCheckIns.length >= 10;
      case 'visits_25': return userCheckIns.length >= 25;
      case 'visits_50': case 'check_ins_50': return userCheckIns.length >= 50;
      case 'visits_100': return userCheckIns.length >= 100;
      case 'streak_7': return calculateCurrentStreak(userCheckIns) >= 7;
      case 'streak_30': return calculateCurrentStreak(userCheckIns) >= 30;
      case 'streak_90': return calculateCurrentStreak(userCheckIns) >= 90;
      case 'referral_3': case 'referral': case 'referral_10': case 'challenge_winner': return false;
      case 'points': case 'none': default: return true;
    }
  };
  ...
  const getRequirementProgress = (requirement, rewardCreatedDate) => {
    ...
    const userCheckIns = checkIns.filter((c) =>
      c.user_id === currentUser.id && new Date(c.check_in_date) >= new Date(rewardCreatedDate));
    const currentStreak = calculateCurrentStreak(userCheckIns);
    switch (requirement) {
      case 'first_visit': return { current: Math.min(userCheckIns.length, 1), target: 1, percentage: Math.min(userCheckIns.length / 1 * 100, 100) };
      case 'visits_3': return { current: Math.min(userCheckIns.length, 3), target: 3, percentage: Math.min(userCheckIns.length / 3 * 100, 100) };
      case 'visits_5': return { current: Math.min(userCheckIns.length, 5), target: 5, percentage: Math.min(userCheckIns.length / 5 * 100, 100) };
      case 'visits_10': case 'check_ins_10': return { current: Math.min(userCheckIns.length, 10), target: 10, percentage: Math.min(userCheckIns.length / 10 * 100, 100) };
      case 'visits_25': return { current: Math.min(userCheckIns.length, 25), target: 25, percentage: Math.min(userCheckIns.length / 25 * 100, 100) };
      case 'visits_50': case 'check_ins_50': return { current: Math.min(userCheckIns.length, 50), target: 50, percentage: Math.min(userCheckIns.length / 50 * 100, 100) };
      case 'visits_100': return { current: Math.min(userCheckIns.length, 100), target: 100, percentage: Math.min(userCheckIns.length / 100 * 100, 100) };
      case 'streak_7': return { current: Math.min(currentStreak, 7), target: 7, percentage: Math.min(currentStreak / 7 * 100, 100) };
      case 'streak_30': return { current: Math.min(currentStreak, 30), target: 30, percentage: Math.min(currentStreak / 30 * 100, 100) };
      case 'streak_90': return { current: Math.min(currentStreak, 90), target: 90, percentage: Math.min(currentStreak / 90 * 100, 100) };
      case 'referral_3': case 'referral': return { current: 0, target: 3, percentage: 0 };
      case 'referral_10': return { current: 0, target: 10, percentage: 0 };
      case 'challenge_winner': return { current: 0, target: 1, percentage: 0 };
      case 'points': case 'none': default: return { current: 1, target: 1, percentage: 100 };
    }
  };

  const calculateCurrentStreak = (userCheckIns) => {
    if (userCheckIns.length === 0) return 0;
    const sortedCheckIns = [...userCheckIns].sort((a, b) => new Date(b.check_in_date) - new Date(a.check_in_date));
    let streak = 1;
    ...
    for (let i = 1; i < sortedCheckIns.length; i++) {
      ...
      if (daysDiff === 1) { streak++; currentDate = checkInDate; }
  ...
  const banMemberMutation = useMutation({
    mutationFn: (userId) => {
      ...
      return base44.entities.Gym.update(gymId, { banned_members: [...currentBanned, userId] });
    },
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['gym', gymId] }); }
  });

  const unbanMemberMutation = useMutation({
    mutationFn: (userId) => {
      const currentBanned = gym?.banned_members || [];
      return base44.entities.Gym.update(gymId, { banned_members: currentBanned.filter((id) => id !== userId) });
    },
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['gym', gymId] }); }
  });
  ...
  const checkInLeaderboard = Object.values(
    weeklyCheckIns.reduce((acc, checkIn) => {
      ...
      if (!acc[userId]) { acc[userId] = { userId, userName: checkIn.user_name, count: 0 }; }
      acc[userId].count++;
      return acc;
    }, {})
  ...
  const streakLeaderboard = Object.values(
    checkIns.reduce((acc, checkIn) => {
      const userId = checkIn.user_id;
      if (!acc[userId]) { acc[userId] = { userId, userName: checkIn.user_name, streak: Math.floor(Math.random() * 30) + 1 }; }
      return acc;
    }, {})
  ...
  const progressLeaderboard = Object.values(
    lifts.reduce((acc, lift) => {
      ...
      if (liftDate >= weekAgoDate) {
        ...
        if (!acc[key]) {
          acc[key] = { userId, userName: lift.member_name, exercise: lift.exercise, maxWeight: lift.weight_lbs, previousMax: 0 };
        } else {
          if (lift.weight_lbs > acc[key].maxWeight) { acc[key].previousMax = acc[key].maxWeight; acc[key].maxWeight = lift.weight_lbs; }
        }
      }
      return acc;
    }, {})
  ).map((item) => ({ userId: item.userId, userName: item.userName, exercise: item.exercise, increase: item.maxWeight - item.previousMax }))
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full overflow-x-hidden">
          ...
          <div className="relative overflow-hidden">
            ...
            <div className="absolute inset-0 z-0">
              ...
              <div className="absolute bottom-0 left-0 right-0 h-16 backdrop-blur-[2px] bg-slate-900/40 [mask-image:linear-gradient(to_top,black_40%,transparent)]" />
            </div>
          ...
          <div className="max-w-4xl mx-auto px-2 md:px-4 py-2 md:py-4 pb-24 w-full overflow-hidden">
              ...
              <motion.div
                ...
                <Card className="bg-gradient-to-br from-slate-900/65 via-slate-900/55 to-slate-950/65 backdrop-blur-3xl border border-white/30 p-3 md:p-5 shadow-2xl shadow-black/30 transition-all duration-300">
                  ...
                  <div className="flex gap-1 md:gap-2 mb-2 md:mb-4 overflow-x-auto pb-2">
                    <Button variant={leaderboardView === 'checkins' ? 'default' : 'outline'} onClick={() => setLeaderboardView('checkins')} size="sm"
                      className={`rounded-2xl whitespace-nowrap text-xs md:text-sm px-2 md:px-3 h-7 md:h-9 font-semibold ${leaderboardView === 'checkins' ? 'bg-gradient-to-r from-green-500 to-emerald-500 text-white border-0' : 'bg-black border-slate-600 text-slate-300'}`}>
                      <CheckCircle className="w-2.5 md:w-3 h-2.5 md:h-3 mr-0.5 md:mr-1" />
                      <span className="hidden sm:inline">Weekly </span>Check-ins
                    </Button>
                    <Button variant={leaderboardView === 'streaks' ? 'default' : 'outline'} onClick={() => setLeaderboardView('streaks')} size="sm"
                      className={`rounded-2xl whitespace-nowrap text-xs md:text-sm px-2 md:px-3 h-7 md:h-9 font-semibold ${leaderboardView === 'streaks' ? 'bg-gradient-to-r from-orange-500 to-red-500 text-white border-0' : 'bg-black border-slate-600 text-slate-300'}`}>
                      <Flame className="w-2.5 md:w-3 h-2.5 md:h-3 mr-0.5 md:mr-1" />Streaks
                    </Button>
                    <Button variant={leaderboardView === 'progress' ? 'default' : 'outline'} onClick={() => setLeaderboardView('progress')} size="sm"
                      className={`rounded-2xl whitespace-nowrap text-xs md:text-sm px-2 md:px-3 h-7 md:h-9 font-semibold ${leaderboardView === 'progress' ? 'bg-gradient-to-r from-cyan-500 to-blue-500 text-white border-0' : 'bg-black border-slate-600 text-slate-300'}`}>
                      <TrendingUp className="w-2.5 md:w-3 h-2.5 md:h-3 mr-0.5 md:mr-1" />Progress
                    </Button>
                  </div>

                  {leaderboardView === 'checkins' && (
                    checkInLeaderboard.length === 0 ?
                    <div className="p-8 text-center"><CheckCircle className="w-12 h-12 mx-auto mb-2 text-slate-600" /><p className="text-slate-400 text-sm">No check-ins this week yet</p></div> :
                    <div className="space-y-1.5 md:space-y-2">
                      {checkInLeaderboard.slice(0, 3).map((member, idx) =>
                        <div key={member.userId} className={`flex items-center gap-2 md:gap-3 p-2 md:p-3 rounded-xl border-2 transition-all backdrop-blur-xl ${idx === 0 ? 'bg-gradient-to-r from-yellow-500/20 to-amber-500/20 border-yellow-400/50 shadow-lg shadow-yellow-500/20' : idx === 1 ? 'bg-gradient-to-r from-gray-400/20 to-gray-500/20 border-gray-400/50 shadow-lg shadow-gray-400/20' : 'bg-gradient-to-r from-orange-500/20 to-red-500/20 border-orange-400/50 shadow-lg shadow-orange-500/20'}`}>
                          <div className={`w-6 md:w-8 h-6 md:h-8 rounded-full flex items-center justify-center font-bold text-white text-xs md:text-sm shadow-lg ${idx === 0 ? 'bg-gradient-to-br from-yellow-400 to-yellow-600' : idx === 1 ? 'bg-gradient-to-br from-gray-300 to-gray-500' : 'bg-gradient-to-br from-orange-500 to-red-600'}`}>{idx + 1}</div>
                          <div className="flex-1"><p className="font-semibold text-white text-xs md:text-sm">{member.userName}</p><p className="text-xs text-slate-300">{member.count} check-ins</p></div>
                  ...
                  {leaderboardView === 'streaks' && (
                    ...
                    <div className="p-8 text-center"><Flame className="w-12 h-12 mx-auto mb-2 text-slate-600" /><p className="text-slate-400 text-sm">No streaks yet</p></div> :
                    <div className="space-y-1.5 md:space-y-2">
                      {streakLeaderboard.slice(0, 3).map((member, idx) =>
                        <div key={member.userId} className={`flex items-center gap-2 md:gap-3 p-2 md:p-3 rounded-xl border-2 transition-all backdrop-blur-xl ${idx === 0 ? 'bg-gradient-to-r from-yellow-500/20 to-amber-500/20 border-yellow-400/50 shadow-lg shadow-yellow-500/20' : idx === 1 ? 'bg-gradient-to-r from-gray-400/20 to-gray-500/20 border-gray-400/50 shadow-lg shadow-gray-400/20' : 'bg-gradient-to-r from-orange-500/20 to-red-500/20 border-orange-400/50 shadow-lg shadow-orange-500/20'}`}>
                          <div className={`w-6 md:w-8 h-6 md:h-8 rounded-full flex items-center justify-center font-bold text-white text-xs md:text-sm shadow-lg ${idx === 0 ? 'bg-gradient-to-br from-yellow-400 to-yellow-600' : idx === 1 ? 'bg-gradient-to-br from-gray-300 to-gray-500' : 'bg-gradient-to-br from-orange-500 to-red-600'}`}>{idx + 1}</div>
                          <div className="flex-1"><p className="font-semibold text-white text-xs md:text-sm">{member.userName}</p><p className="text-xs text-slate-300">{member.streak}d streak</p></div>
                  ...
                  {leaderboardView === 'progress' && (
                    ...
                    <div className="p-8 text-center"><TrendingUp className="w-12 h-12 mx-auto mb-2 text-slate-600" /><p className="text-slate-400 text-sm">No progress this week yet</p></div> :
                    <div className="space-y-1.5 md:space-y-2">
                      {progressLeaderboard.slice(0, 3).map((member, idx) =>
                        <div key={`${member.userId}-${member.exercise}`} className={`flex items-center gap-2 md:gap-3 p-2 md:p-3 rounded-xl border-2 transition-all backdrop-blur-xl ${idx === 0 ? 'bg-gradient-to-r from-yellow-500/20 to-amber-500/20 border-yellow-400/50 shadow-lg shadow-yellow-500/20' : idx === 1 ? 'bg-gradient-to-r from-gray-400/20 to-gray-500/20 border-gray-400/50 shadow-lg shadow-gray-400/20' : 'bg-gradient-to-r from-orange-500/20 to-red-500/20 border-orange-400/50 shadow-lg shadow-orange-500/20'}`}>
                          <div className={`w-6 md:w-8 h-6 md:h-8 rounded-full flex items-center justify-center font-bold text-white text-xs md:text-sm shadow-lg ${idx === 0 ? 'bg-gradient-to-br from-yellow-400 to-yellow-600' : idx === 1 ? 'bg-gradient-to-br from-gray-300 to-gray-500' : 'bg-gradient-to-br from-orange-500 to-red-600'}`}>{idx + 1}</div>
                          <div className="flex-1"><p className="font-semibold text-white text-xs md:text-sm">{member.userName}</p><p className="text-xs text-slate-300">{member.exercise.replace('_', ' ')} • +{member.increase}kg</p></div>
            ...
            <TabsContent value="feed" className="space-y-3 mt-0 w-full overflow-hidden" asChild>
              <motion.div
                ...
                transition={{ duration: 0.3, ease: [0.34, 1.56, 0.64, 1] }} className="space-y-3">

                {upcomingEvents.length > 0 &&
                  <div className="space-y-3">
                    <div className="flex items-center gap-2"><h2 className="text-base md:text-lg font-bold text-white">📅 This Week</h2></div>
                ...
                {showOwnerControls &&
                  <CreateGymPostButton gym={gym} currentUser={currentUser} onPostCreated={() => queryClient.invalidateQueries({ queryKey: ['posts'] })} />
                }
            ...
            <TabsContent value="challenges" className="space-y-2 md:space-y-4 mt-0 w-full overflow-hidden" asChild>
              <motion.div
                initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}
                transition={{ duration: 0.3, ease: [0.34, 1.56, 0.64, 1] }} className="space-y-2 md:space-y-4">
                  ...
                  <div className="space-y-3">
                    {gymChallenges.map((challenge) =>
                      <GymChallengeCard key={challenge.id} challenge={challenge}
                        ...
                        onDelete={showOwnerControls ? (challengeId) => { if (window.confirm('Delete this challenge?')) { deleteChallengeMutation.mutate(challengeId); } } : null} />
                    )}
                  </div>
                }
            ...
            <TabsContent value="events" className="space-y-2 md:space-y-3 mt-0 w-full overflow-hidden" asChild>
              <motion.div
                initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}
                transition={{ duration: 0.3, ease: [0.34, 1.56, 0.64, 1] }} className="space-y-2 md:space-y-3">
                ...
                <Card className="bg-gradient-to-br from-slate-900/65 via-slate-900/55 to-slate-950/65 backdrop-blur-3xl border border-white/30 p-2 md:p-5 shadow-2xl shadow-black/30">
                  <div className="flex items-center justify-between mb-2 md:mb-4">
                    ...
                    {showOwnerControls && <Button onClick={() => setShowManageClasses(true)} size="sm" variant="outline" className="rounded-2xl"><Edit className="w-3 h-3 mr-1" />Manage</Button>}
                  </div>
                  {classes.length === 0 ?
                    <div className="p-4 text-center border-2 border-dashed border-slate-600/50 rounded-2xl"><Calendar className="w-8 h-8 mx-auto mb-1 text-slate-500" /><p className="text-slate-400 text-xs">No classes scheduled</p></div> :
                        ...
                        <div key={gymClass.id} className="bg-slate-700/50 border border-slate-600/40 p-2 md:p-4 rounded-2xl hover:bg-slate-700/70 transition-all flex items-start justify-between gap-3">
                          <div className="flex items-start gap-2 md:gap-3 flex-1 min-w-0">
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="flex flex-wrap gap-1 mb-2">
                                <Badge className="bg-purple-500/20 text-purple-300 border-purple-500/30 text-xs">{gymClass.instructor}</Badge>
                                {gymClass.duration_minutes && <Badge variant="outline" className="text-xs bg-slate-600/50 text-slate-200 border-slate-500">{gymClass.duration_minutes} min</Badge>}
                              </div>
                                ...
                                <div className="space-y-1">
                                  {gymClass.schedule.map((slot, idx) =>
                                    <div key={idx} className="text-xs text-slate-300 flex items-center gap-1">
                                      <Calendar className="w-3 h-3" /><span className="font-medium">{slot.day}</span> • <span>{slot.time}</span>
                                    </div>
                                  )}
                                </div>
                              }
                          ...
                          {showOwnerControls &&
                            <Button onClick={() => { if (window.confirm('Delete this class?')) { deleteClassMutation.mutate(gymClass.id); } }}
                              variant="outline" size="icon" className="border-red-500/50 hover:bg-red-500/10 hover:border-red-500 flex-shrink-0 min-h-[44px] min-w-[44px]">
                ...
                <Card className="bg-gradient-to-br from-slate-900/65 via-slate-900/55 to-slate-950/65 backdrop-blur-3xl border border-white/30 p-2 md:p-5 shadow-2xl shadow-black/30">
                  <div className="flex items-center justify-between mb-2 md:mb-4">
                    ...
                    {showOwnerControls && <Button onClick={() => setShowCreateEvent(true)} size="sm" className="bg-blue-500 text-white rounded-2xl"><Plus className="w-4 h-4 mr-1" />Create</Button>}
                  </div>
                  {events.filter((e) => new Date(e.event_date) >= new Date()).length === 0 ?
                    <div className="p-4 text-center border-2 border-dashed border-slate-600/50 rounded-2xl"><Calendar className="w-8 h-8 mx-auto mb-1 text-slate-500" /><p className="text-slate-400 text-xs">No upcoming events</p></div> :
                    <div className="space-y-2 md:space-y-3">
                      {events.filter((e) => new Date(e.event_date) >= new Date()).slice(0, 5).map((event) =>
                        <EventCard key={event.id} event={event}
                          ...
                          onDelete={showOwnerControls ? (eventId) => { if (window.confirm('Delete this event?')) { deleteEventMutation.mutate(eventId); } } : null} />
                      )}
                    </div>
                  }
                </Card>
                ...
                <Card className="bg-gradient-to-br from-slate-900/65 via-slate-900/55 to-slate-950/65 backdrop-blur-3xl border border-white/30 p-2 md:p-5 shadow-2xl shadow-black/30">
                  <div className="flex items-center justify-between mb-2 md:mb-4">
                    ...
                    {showOwnerControls && <Button onClick={() => setShowManageCoaches(true)} size="sm" variant="outline" className="rounded-2xl"><Edit className="w-3 h-3 mr-1" />Manage</Button>}
                  </div>
                  {coaches.length === 0 ?
                    <div className="p-4 text-center border-2 border-dashed border-slate-600/50 rounded-2xl"><GraduationCap className="w-8 h-8 mx-auto mb-1 text-slate-500" /><p className="text-slate-400 text-xs">No coaches listed</p></div> :
                    <div className="space-y-2 md:space-y-3">
                          ...
                          <div key={coach.id} className="bg-slate-700/50 border border-slate-600/40 p-2 md:p-4 rounded-2xl hover:bg-slate-700/70 transition-all">
                            <div className="flex items-start gap-2 md:gap-3">
                              <div className="w-10 md:w-12 h-10 md:h-12 rounded-full bg-gradient-to-br from-blue-500 to-cyan-500 flex items-center justify-center flex-shrink-0 overflow-hidden">
                                {coach.avatar_url ? <img src={coach.avatar_url} alt={coach.name} className="w-full h-full object-cover" /> : <span className="text-lg font-bold text-white">{coach.name.charAt(0)}</span>}
                              </div>
                              <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-1.5 md:gap-2 mb-0.5 md:mb-1">
                                  <h4 className="font-semibold text-white text-sm md:text-base line-clamp-1">{coach.name}</h4>
                                  {coach.rating && <div className="flex items-center gap-0.5"><Star className="w-2.5 md:w-3 h-2.5 md:h-3 fill-yellow-400 text-yellow-400 flex-shrink-0" /><span className="text-[10px] md:text-xs font-bold text-slate-200">{coach.rating}</span></div>}
                                </div>
                                {coach.bio && <p className="text-xs text-slate-300 mb-1 md:mb-2 line-clamp-1">{coach.bio}</p>}
                                {coach.specialties && coach.specialties.length > 0 &&
                                  <div className="flex flex-wrap gap-1">
                                    {coach.specialties.map((specialty, idx) => <Badge key={idx} className="bg-blue-500/20 text-blue-300 border-blue-500/30 text-xs">{specialty}</Badge>)}
                                  </div>
                                }
                              </div>
                              <Popover>
                                ...
                                <PopoverContent className="w-auto p-3 bg-slate-800 border-slate-700">
                                  <div className="flex items-center gap-2">
                                    <a href={`mailto:${coach.user_email}`} className="text-blue-400 hover:text-blue-300 text-sm font-medium break-all flex-1">{coach.user_email}</a>
                                    <button onClick={handleCopyEmail} className="min-h-[44px] min-w-[44px] flex items-center justify-center hover:bg-slate-700 rounded transition-colors flex-shrink-0" title="Copy email">
