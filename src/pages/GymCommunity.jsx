...
export default function GymCommunity() {
  ...
  const { data: memberPosts = [] } = useQuery({ queryKey: ['memberPosts', gymId], queryFn: () => base44.entities.Post.filter({ allow_gym_repost: true }, '-created_date', 20), enabled: !!gymId, staleTime: 2 * 60 * 1000, gcTime: 10 * 60 * 1000, placeholderData: (prev) => prev });
  const { data: gymPosts = [] } = useQuery({ queryKey: ['gymPosts', gymId], queryFn: () => base44.entities.Post.filter({ member_id: gymId }, '-created_date', 20), enabled: !!gymId, staleTime: 2 * 60 * 1000, gcTime: 10 * 60 * 1000, placeholderData: (prev) => prev });
  const posts = React.useMemo(() => [...gymPosts, ...memberPosts].sort((a, b) => new Date(b.created_date) - new Date(a.created_date)), [gymPosts, memberPosts]);
