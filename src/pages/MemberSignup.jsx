...
import {
  ...
  QrCode, Camera, Upload, BarChart2
} from 'lucide-react';
...
export default function GymSignup() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setCreatedGym(gym);
      toast.success(isVerified ? 'Gym registered and live!' : "Gym submitted! We'll review it within 24 hours.");
      setStep(7);
    } catch (err) {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-900 via-slate-900 to-blue-950 flex flex-col items-center justify-between p-6 relative overflow-hidden">
        ...
        <div className="flex-1 flex flex-col items-center justify-center relative z-10 text-center max-w-sm px-4">
          <div
            className={`w-28 h-28 rounded-3xl bg-gradient-to-br ${slide.color} flex items-center justify-center mb-8 shadow-2xl transition-all duration-500`}
        ...
        <div className="relative z-10 w-full max-w-sm flex flex-col items-center gap-5">
          ...
          <button
            onClick={() => navigate(createPageUrl('MemberSignup'))}
          ...
          <div className="fixed inset-0 bg-black/75 backdrop-blur-md z-50 flex items-center justify-center p-4">
            <div className="max-w-sm w-full bg-gradient-to-b from-slate-800 to-slate-900 border border-white/10 rounded-3xl p-6 shadow-2xl">
              ...
              <div className="flex gap-3">
                <button onClick={() => setShowGhostModal(false)}
  ...
  if (step === 3) {
    return (
      <PageWrapper>
        <StepHeader step={3} title="Verify Ownership" subtitle={`Prove you own ${formData.name}`} />
        <div className="bg-white/5 backdrop-blur-2xl border border-white/8 rounded-3xl p-6 shadow-2xl">
          <div className="flex gap-2 p-1 bg-black/20 rounded-2xl mb-6 border border-white/5">
            <button
              onClick={() => { setVerificationMethod('email'); setCodeSent(false); setEmailVerified(false); setEnteredCode(''); }}
            ...
            <button
              onClick={() => setVerificationMethod('instagram')}
              className={`flex-1 py-3 rounded-xl text-sm font-bold transition-all flex items-center justify-center gap-2 ${
  ...
  if (step === 4) {
    return (
      <PageWrapper>
        <StepHeader step={4} title="Set Up Your Gym" subtitle="Tell members what makes your gym great" />
        <div className="bg-white/5 backdrop-blur-2xl border border-white/8 rounded-3xl p-6 shadow-2xl space-y-6">

          {/* Cover photo */}
          <div>
            <Label className="text-slate-300 font-semibold text-xs uppercase tracking-wider mb-2 block">Cover Photo</Label>
            <button
              type="button"
              onClick={() => coverRef.current?.click()}
              className="w-full h-32 rounded-2xl border-2 border-dashed border-white/15 hover:border-blue-400/40 bg-white/3 hover:bg-white/5 transition-all flex flex-col items-center justify-center gap-2 overflow-hidden relative"
            >
              {coverPreview ? (
            ...
            {formData.equipment.length > 0 ? (
              <div className="flex flex-wrap gap-2">
                {formData.equipment.map((item) => (
                  <div key={item} className="inline-flex items-center gap-1.5 bg-blue-500/15 border border-blue-400/25 text-blue-200 text-xs font-semibold px-3 py-1.5 rounded-xl">
                    {item}
