...
function RankPage({ currentUser, onBack }) {
  const badges = [
    { id: 1, name: 'First Rep', description: 'Logged your first workout', icon: '🏋️', earned: true, color: 'from-yellow-500/20 to-orange-500/10', border: 'border-yellow-500/30', textColor: 'text-yellow-400' },
    { id: 2, name: 'Week Warrior', description: '7-day training streak', icon: '🔥', earned: true, color: 'from-orange-500/20 to-red-500/10', border: 'border-orange-500/30', textColor: 'text-orange-400' },
    { id: 3, name: 'PR Hunter', description: 'Set a new personal record', icon: '🎯', earned: true, color: 'from-purple-500/20 to-pink-500/10', border: 'border-purple-500/30', textColor: 'text-purple-400' },
    { id: 4, name: 'Iron Will', description: '30-day consistency badge', icon: '⚡', earned: false, color: 'from-slate-700/20 to-slate-800/10', border: 'border-slate-700/30', textColor: 'text-slate-500' },
    { id: 5, name: 'Goal Crusher', description: 'Complete your first goal', icon: '🏆', earned: false, color: 'from-slate-700/20 to-slate-800/10', border: 'border-slate-700/30', textColor: 'text-slate-500' },
    { id: 6, name: 'Community Star', description: 'Check in with 5 gym friends', icon: '⭐', earned: false, color: 'from-slate-700/20 to-slate-800/10', border: 'border-slate-700/30', textColor: 'text-slate-500' },
  ];
    ...
    <SubPage title="Rank" onBack={onBack}>
      ...
      <div className="grid grid-cols-2 gap-2.5">
        {badges.map((badge) => (
          <div
            key={badge.id}
            className={`relative p-3.5 rounded-2xl border bg-gradient-to-br ${badge.color} ${badge.border} ${!badge.earned ? 'opacity-50' : ''}`}
          >
            {badge.earned && (
...
export default function Progress() {
  ...
  if (view === 'rank')      return <RankPage currentUser={currentUser} onBack={() => setView('hub')} />;
