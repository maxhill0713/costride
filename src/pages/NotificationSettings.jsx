  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-900 to-slate-900">
      ...
      <div className="max-w-2xl mx-auto px-4 py-6 space-y-4">
        ...
        {/* Push notifications — id: section-push */}
        <SectionCard sectionId="push" highlightedSection={highlightedSection}>
          <Card className="bg-gradient-to-br from-slate-900/70 via-slate-900/60 to-slate-950/70 backdrop-blur-xl border border-white/10 p-4 shadow-2xl shadow-black/20">
            <div className="flex items-center justify-between p-3 bg-white/5 border border-white/10 rounded-2xl">
              <div className="flex items-center gap-3">
                {currentUser.notifications_enabled ? <Bell className="w-4 h-4 text-slate-400" /> : <BellOff className="w-4 h-4 text-slate-500" />}
                <Label className="text-sm font-semibold text-slate-100">Push Notifications</Label>
              </div>
              <Switch checked={currentUser.notifications_enabled ?? true} onCheckedChange={(checked) => updateSettingsMutation.mutate({ notifications_enabled: checked })} />
            </div>
          </Card>
        </SectionCard>

        {/* Email notifications — id: section-email-notif */}
        <SectionCard sectionId="email-notif" highlightedSection={highlightedSection}>
          <Card className="bg-gradient-to-br from-slate-900/70 via-slate-900/60 to-slate-950/70 backdrop-blur-xl border border-white/10 p-4 shadow-2xl shadow-black/20">
            <div className="flex items-center justify-between p-3 bg-white/5 border border-white/10 rounded-2xl">
              <div className="flex items-center gap-3">
                {currentUser.email_notifications ? <Bell className="w-4 h-4 text-slate-400" /> : <BellOff className="w-4 h-4 text-slate-500" />}
                <Label className="text-sm font-semibold text-slate-100">Email Notifications</Label>
              </div>
              <Switch checked={currentUser.email_notifications ?? true} onCheckedChange={(checked) => updateSettingsMutation.mutate({ email_notifications: checked })} />
            </div>
          </Card>
        </SectionCard>
