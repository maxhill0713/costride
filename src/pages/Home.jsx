              ...
              {friendRequests.filter(req => { const u = friendUsersList.find(u => u.id === req.user_id); return (u?.full_name||req.user_name||'').toLowerCase().includes(friendsListSearchQuery.toLowerCase()); }).map(request => {
                ...
                return (
                  <div key={request.id} className="p-3 rounded-lg bg-blue-700/40 border border-blue-500/30 flex items-center justify-between gap-3">
                    ...
                    <Badge className="bg-blue-500/20 text-blue-300 border-blue-500/40 text-[10px] flex-shrink-0">Request pending</Badge>
                      ...
                      <div key={user.id} className="flex items-center justify-between p-3 bg-slate-700/50 rounded-xl">
                        <div className="flex items-center gap-3">
                          <div className="w-10 h-10 rounded-full bg-gradient-to-br from-slate-600 to-slate-700 flex items-center justify-center overflow-hidden">
                            {user.avatar_url ? <img src={user.avatar_url} alt={user.full_name} className="w-full h-full object-cover rounded-full" /> : <span className="text-sm font-semibold text-white">{(user.display_name || user.username || user.full_name)?.charAt(0)?.toUpperCase()}</span>}
                          </div>
