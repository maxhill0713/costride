...
export default function GymOwnerDashboard() {
    ...
    if (currentUser?.id && window.OneSignalDeferred) {
      window.OneSignalDeferred.push(async (OneSignal) => {
        try {
          ...
          console.log('OneSignal login successful for:', currentUser.id);
        } catch (error) {
  ...
  const { data: posts = [] } = useQuery({ queryKey: ['posts', selectedGym?.id, currentUser?.id], queryFn: async () => {
    // Fetch all posts for the gym (RLS will filter based on user role)
    return base44.entities.Post.filter({ gym_id: selectedGym.id }, '-created_date', 50);
  }, enabled: on, ...qo });
  ...
  const deletePostM = useMutation({
    mutationFn: async ({ id, reason }) => {
      // Fetch post first to get member_id for notification
      const posts = await base44.entities.Post.filter({ id });
      const post = posts[0];
      await base44.entities.Post.delete(id);
      if (post?.member_id && reason) {
        await base44.entities.Notification.create({
          user_id: post.member_id,
          type: 'post_removed',
          title: 'Post Removed',
          message: `Your post was removed by your gym. Reason: ${reason}`,
          read: false,
        });
      }
    },
  ...
  const updatePostM = useMutation({ mutationFn: ({ id, data }) => base44.entities.Post.update(id, data), onSuccess: () => inv('posts'), onError: onErr });
  ...
  const tabPanels = NAV.map((item) => {
    ...
    if (item.id === 'overview' && !isCoach) {
      ...
      content = isCoach ?
        ...
        <TabContentComponent events={events} challenges={challenges} polls={polls} posts={posts} classes={classes} checkIns={checkIns} ci30={ci30} avatarMap={avatarMapFull} nameMap={memberNameMap} currentUser={currentUser} gym={selectedGym} leaderboardView={leaderboardView} setLeaderboardView={setLeaderboardView} openModal={openModal} now={now} onDeletePost={(id, reason) => deletePostM.mutate({ id, reason })} onUpdatePost={() => inv('posts')} onDeleteEvent={(id) => deleteEventM.mutate(id)} onUpdateEvent={() => inv('events')} onDeleteChallenge={(id) => deleteChallengeM.mutate(id)} onDeleteClass={(id) => deleteClassM.mutate(id)} onDeletePoll={(id) => deletePollM.mutate(id)} memberCount={totalMembers} memberUserRecords={memberUserRecords} />;
      ...
      content = <TabGymProfile gym={selectedGym} openModal={openModal} coaches={coaches} />;
  ...
  const sharedModals = (
    <>
      ...
      <CreatePollModal open={modal === 'poll'} onClose={closeModal} onSave={(d) => createPollM.mutate(d)} isLoading={createPollM.isPending} gym={selectedGym} />
