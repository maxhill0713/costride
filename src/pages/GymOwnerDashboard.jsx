...
function TabCoachOverview({ myClasses, checkIns, allMemberships, avatarMap, openModal, now, selectedGym, posts, events, challenges, polls }) {
  const todayCheckIns = checkIns.filter(c => startOfDay(new Date(c.check_in_date)).getTime() === startOfDay(now).getTime());
  const weekCheckIns  = checkIns.filter(c => isWithinInterval(new Date(c.check_in_date), { start: subDays(now, 7), end: now }));
  const ci30          = checkIns.filter(c => isWithinInterval(new Date(c.check_in_date), { start: subDays(now, 30), end: now }));

  // Members who attended coach's classes recently
  const activeClassMembers = new Set(weekCheckIns.map(c => c.user_id)).size;
  const totalMembers       = allMemberships.length;

  // At-risk: haven't checked in last 14 days
  const memberLastCI = {};
  ...
  const atRiskMembers = allMemberships.filter(m => { const l = memberLastCI[m.user_id]; return !l || Math.floor((now - new Date(l)) / 86400000) >= 14; });

  // Upcoming events coach created
  const myEvents     = events.filter(e => new Date(e.event_date) >= now).slice(0, 3);
  const activeChal   = challenges.filter(c => c.status === 'active');
  const recentPosts  = [...posts].slice(0, 5);
  const recentActivity = [...checkIns].slice(0, 6).map(c => ({ name: c.user_name || 'Member', user_id: c.user_id, time: c.check_in_date }));

  // Spark for today's checkins
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 280px', gap: 18, alignItems: 'start' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12 }}>
          <CoachKpiCard icon={Activity}   label="Today's Check-ins"  value={todayCheckIns.length}    sub="members in today"         accentColor="#10b981" subColor={todayCheckIns.length > 0 ? '#34d399' : '#64748b'} footerBar={totalMembers > 0 ? (todayCheckIns.length / totalMembers) * 100 : 0}/>
          <CoachKpiCard icon={Users}      label="Active This Week"   value={activeClassMembers}       sub={`of ${totalMembers} members`} accentColor="#0ea5e9" footerBar={totalMembers > 0 ? (activeClassMembers / totalMembers) * 100 : 0}/>
          <CoachKpiCard icon={Dumbbell}   label="My Classes"         value={myClasses.length}          sub="in your schedule"          accentColor="#a78bfa"/>
          <CoachKpiCard icon={AlertCircle} label="At Risk"           value={atRiskMembers.length}     sub="14+ days inactive"         accentColor={atRiskMembers.length > 0 ? '#ef4444' : '#10b981'} subColor={atRiskMembers.length > 0 ? '#f87171' : '#34d399'}/>
        </div>

        {/* My classes today */}
        <CoachCard accent="#a78bfa" style={{ padding: 20 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
            <div>
              <div style={{ fontSize: 14, fontWeight: 800, color: '#f0f4f8', letterSpacing: '-0.02em' }}>My Classes</div>
              <div style={{ fontSize: 11, color: '#64748b', marginTop: 2 }}>Your full class schedule</div>
            </div>
            <button onClick={() => openModal('classes')} style={{ padding: '5px 12px', borderRadius: 8, background: 'rgba(167,139,250,0.1)', border: '1px solid rgba(167,139,250,0.2)', color: '#a78bfa', fontSize: 11, fontWeight: 700, cursor: 'pointer' }}>
              Manage
            </button>
          </div>
          {myClasses.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '24px 0', color: '#3a5070' }}>
              <Dumbbell style={{ width: 24, height: 24, opacity: 0.3, margin: '0 auto 8px' }}/>
              <p style={{ fontSize: 12, fontWeight: 600, margin: 0 }}>No classes in your schedule yet</p>
            </div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {myClasses.map((cls, i) => {
                const typeColors = { hiit: '#f87171', yoga: '#34d399', strength: '#818cf8', spin: '#38bdf8', boxing: '#fb923c', cardio: '#fb7185', default: '#a78bfa' };
                const n = (cls.class_type || cls.name || '').toLowerCase();
                const colorKey = Object.keys(typeColors).find(k => n.includes(k)) || 'default';
                const c = typeColors[colorKey];
                return (
                  <div key={cls.id || i} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 12px', borderRadius: 12, background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.05)' }}>
                    <div style={{ width: 36, height: 36, borderRadius: 10, background: `${c}18`, border: `1px solid ${c}28`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                      <Dumbbell style={{ width: 14, height: 14, color: c }}/>
                    </div>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: 13, fontWeight: 700, color: '#f0f4f8', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{cls.name || cls.title}</div>
                      <div style={{ display: 'flex', gap: 8, marginTop: 2 }}>
                        ...
                        {cls.capacity && <span style={{ fontSize: 10, color: '#64748b' }}>· {cls.capacity} max</span>}
                      </div>
                    </div>
                    <span style={{ fontSize: 9, fontWeight: 800, padding: '2px 7px', borderRadius: 5, background: `${c}15`, color: c, textTransform: 'uppercase', letterSpacing: '0.06em', flexShrink: 0 }}>{colorKey}</span>
                  </div>
                );
              })}
            </div>
        ...
        {/* Recent check-ins + At-risk */}
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
          <CoachCard accent="#38bdf8" style={{ padding: 18 }}>
            <div style={{ fontSize: 13, fontWeight: 800, color: '#f0f4f8', marginBottom: 14, letterSpacing: '-0.01em' }}>Recent Check-ins</div>
            {recentActivity.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '16px 0', color: '#3a5070', fontSize: 11, fontWeight: 600 }}>No activity yet</div>
            ) : (
                ...
                {recentActivity.map((a, i) => {
                  ...
                  return (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
                      <div style={{ width: 28, height: 28, borderRadius: '50%', background: avatarMap[a.user_id] ? 'transparent' : 'linear-gradient(135deg,#0ea5e9,#8b5cf6)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 800, color: '#fff', flexShrink: 0, overflow: 'hidden' }}>
                        {avatarMap[a.user_id] ? <img src={avatarMap[a.user_id]} style={{ width: '100%', height: '100%', objectFit: 'cover' }}/> : (a.name || '?').charAt(0).toUpperCase()}
                      </div>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontSize: 12, fontWeight: 700, color: '#f0f4f8', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{a.name}</div>
                        <div style={{ fontSize: 10, color: '#3a5070' }}>{timeStr}</div>
                      </div>
          ...
          <CoachCard accent="#ef4444" style={{ padding: 18 }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
              <div style={{ fontSize: 13, fontWeight: 800, color: '#f0f4f8', letterSpacing: '-0.01em' }}>At-Risk Members</div>
              {atRiskMembers.length > 0 && (
                <span style={{ fontSize: 10, fontWeight: 700, color: '#f87171', background: 'rgba(248,113,113,0.1)', border: '1px solid rgba(248,113,113,0.2)', borderRadius: 5, padding: '2px 6px' }}>{atRiskMembers.length}</span>
              )}
            </div>
            {atRiskMembers.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '16px 0' }}>
                <CheckCircle style={{ width: 22, height: 22, color: '#34d399', margin: '0 auto 6px' }}/>
                <div style={{ fontSize: 12, fontWeight: 700, color: '#34d399' }}>All members active</div>
              </div>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {atRiskMembers.slice(0, 5).map((m, i) => {
                  ...
                  return (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
                      <div style={{ width: 28, height: 28, borderRadius: '50%', background: avatarMap[m.user_id] ? 'transparent' : 'rgba(248,113,113,0.1)', border: '1px solid rgba(248,113,113,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 800, color: '#f87171', flexShrink: 0, overflow: 'hidden' }}>
                        {avatarMap[m.user_id] ? <img src={avatarMap[m.user_id]} style={{ width: '100%', height: '100%', objectFit: 'cover' }}/> : (m.user_name || '?').charAt(0).toUpperCase()}
                      </div>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <div style={{ fontSize: 10, color: '#f87171' }}>{daysAgo ? `${daysAgo}d absent` : 'Never checked in'}</div>
                      </div>
                      <button onClick={() => openModal('post')} style={{ fontSize: 9, fontWeight: 700, color: '#38bdf8', background: 'rgba(56,189,248,0.08)', border: '1px solid rgba(56,189,248,0.16)', borderRadius: 5, padding: '3px 7px', cursor: 'pointer', flexShrink: 0 }}>Reach</button>
                    </div>
                  );
                })}
              </div>
        ...
        {/* Upcoming events */}
        {myEvents.length > 0 && (
          <CoachCard accent="#34d399" style={{ padding: 18 }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
              <div style={{ fontSize: 13, fontWeight: 800, color: '#f0f4f8', letterSpacing: '-0.01em' }}>Upcoming Events</div>
              <button onClick={() => openModal('event')} style={{ padding: '4px 10px', borderRadius: 7, background: 'rgba(52,211,153,0.1)', border: '1px solid rgba(52,211,153,0.2)', color: '#34d399', fontSize: 11, fontWeight: 700, cursor: 'pointer' }}>
                + New
              </button>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {myEvents.map((ev, i) => {
                ...
                const diffD   = Math.floor((evDate - now) / 86400000);
                const urgency = diffD <= 2;
                return (
                  <div key={ev.id || i} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 12px', borderRadius: 12, background: 'rgba(52,211,153,0.04)', border: '1px solid rgba(52,211,153,0.1)' }}>
                    <div style={{ flexShrink: 0, background: 'rgba(52,211,153,0.08)', border: '1px solid rgba(52,211,153,0.14)', borderRadius: 9, padding: '5px 8px', textAlign: 'center', minWidth: 36 }}>
                      <div style={{ fontSize: 15, fontWeight: 900, color: '#34d399', lineHeight: 1 }}>{format(evDate, 'd')}</div>
                      <div style={{ fontSize: 8, fontWeight: 800, color: '#1a5a3a', textTransform: 'uppercase' }}>{format(evDate, 'MMM')}</div>
                    </div>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: 13, fontWeight: 700, color: '#f0f4f8', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{ev.title}</div>
                      <div style={{ fontSize: 10, color: '#64748b' }}>{format(evDate, 'h:mm a')}</div>
                    </div>
                    <span style={{ fontSize: 9, fontWeight: 700, color: urgency ? '#f87171' : '#34d399', background: urgency ? 'rgba(248,113,113,0.1)' : 'rgba(52,211,153,0.1)', borderRadius: 5, padding: '2px 6px', flexShrink: 0 }}>
                      {diffD === 0 ? 'Today' : diffD === 1 ? 'Tomorrow' : `${diffD}d`}
                    </span>
                  </div>
                );
              })}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        {/* Quick actions */}
        <CoachCard accent="#38bdf8" style={{ padding: 16 }}>
          <div style={{ fontSize: 12, fontWeight: 800, color: '#f0f4f8', marginBottom: 12, letterSpacing: '-0.01em' }}>Quick Actions</div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 7 }}>
            {[
              { icon: QrCode,            label: 'Scan Check-in',  color: '#10b981', fn: () => openModal('qrScanner')  },
              { icon: MessageSquarePlus, label: 'Post Update',     color: '#38bdf8', fn: () => openModal('post')       },
              ...
              <button key={i} onClick={fn} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '9px 10px', borderRadius: 10, background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.07)', cursor: 'pointer', transition: 'all 0.14s', textAlign: 'left' }}
                ...
                <div style={{ width: 24, height: 24, borderRadius: 7, background: `${color}18`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
        ...
        {/* 30-Day snapshot */}
        <CoachCard accent="#0ea5e9" style={{ padding: 16 }}>
          <div style={{ fontSize: 12, fontWeight: 800, color: '#f0f4f8', marginBottom: 12, letterSpacing: '-0.01em' }}>30-Day Snapshot</div>
          {[
            { label: 'Total check-ins', value: ci30.length,                                                             color: '#38bdf8' },
            { label: 'Active members',  value: new Set(ci30.map(c => c.user_id)).size,                                  color: '#34d399' },
            { label: 'At-risk members', value: atRiskMembers.length,                                                    color: atRiskMembers.length > 0 ? '#f87171' : '#34d399' },
            { label: 'Active challenges', value: activeChal.length,                                                     color: '#fbbf24' },
          ].map((s, i, arr) => (
            <div key={i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '9px 0', borderBottom: i < arr.length - 1 ? '1px solid rgba(255,255,255,0.04)' : 'none' }}>
              <span style={{ fontSize: 11, color: '#94a3b8', fontWeight: 500 }}>{s.label}</span>
              <span style={{ fontSize: 13, fontWeight: 800, color: s.color, background: `${s.color}12`, border: `1px solid ${s.color}22`, borderRadius: 6, padding: '2px 8px' }}>{s.value}</span>
            </div>
          ))}
        </CoachCard>

        {/* Member attendance breakdown */}
        <CoachCard accent="#8b5cf6" style={{ padding: 16 }}>
          <div style={{ fontSize: 12, fontWeight: 800, color: '#f0f4f8', marginBottom: 12, letterSpacing: '-0.01em' }}>Attendance Breakdown</div>
          {(() => {
            const per = {};
            ci30.forEach(c => { per[c.user_id] = (per[c.user_id] || 0) + 1; });
            const vals = Object.values(per);
            const superActive = vals.filter(v => v >= 12).length;
            const active      = vals.filter(v => v >= 4 && v < 12).length;
            const casual      = vals.filter(v => v >= 1 && v < 4).length;
            const inactive    = Math.max(0, totalMembers - vals.length);
            const tiers = [
              { label: 'Super Active', sub: '12+ visits', val: superActive, color: '#10b981' },
              { label: 'Active',       sub: '4–11',       val: active,      color: '#0ea5e9' },
              { label: 'Casual',       sub: '1–3',        val: casual,      color: '#a78bfa' },
              { label: 'Inactive',     sub: '0 visits',   val: inactive,    color: '#f59e0b' },
            ];
            return tiers.map((t, i) => {
              const pct = totalMembers > 0 ? (t.val / totalMembers) * 100 : 0;
              return (
                <div key={i} style={{ marginBottom: i < tiers.length - 1 ? 10 : 0 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                      <div style={{ width: 6, height: 6, borderRadius: '50%', background: t.color }}/>
                      <span style={{ fontSize: 11, fontWeight: 700, color: '#d4e4f4' }}>{t.label}</span>
                      <span style={{ fontSize: 9, color: '#64748b' }}>{t.sub}</span>
                    </div>
                    <div style={{ display: 'flex', gap: 5 }}>
                      <span style={{ fontSize: 11, fontWeight: 800, color: t.color }}>{t.val}</span>
                      <span style={{ fontSize: 9, color: '#64748b' }}>{Math.round(pct)}%</span>
                    </div>
                  </div>
                  <div style={{ height: 4, borderRadius: 99, background: 'rgba(255,255,255,0.05)', overflow: 'hidden' }}>
                    <div style={{ height: '100%', width: `${pct}%`, background: `linear-gradient(90deg,${t.color},${t.color}99)`, borderRadius: 99, transition: 'width 0.8s ease' }}/>
                  </div>
                </div>
              );
            });
        ...
        {/* Active challenges */}
        {activeChal.length > 0 && (
          <CoachCard accent="#fbbf24" style={{ padding: 16 }}>
            <div style={{ fontSize: 12, fontWeight: 800, color: '#f0f4f8', marginBottom: 12, letterSpacing: '-0.01em' }}>Active Challenges</div>
            {activeChal.slice(0, 3).map((ch, i) => (
              <div key={ch.id || i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 0', borderBottom: i < Math.min(activeChal.length, 3) - 1 ? '1px solid rgba(255,255,255,0.04)' : 'none' }}>
                <div style={{ width: 32, height: 32, borderRadius: 9, background: 'rgba(245,158,11,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                  <Trophy style={{ width: 13, height: 13, color: '#fbbf24' }}/>
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 12, fontWeight: 700, color: '#f0f4f8', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{ch.title}</div>
                  <div style={{ fontSize: 10, color: '#64748b' }}>{(ch.participants || []).length} joined</div>
                </div>
              </div>
            ))}
          </CoachCard>
        ...
        {recentPosts.length > 0 && (
          <CoachCard accent="#34d399" style={{ padding: 16 }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
              <div style={{ fontSize: 12, fontWeight: 800, color: '#f0f4f8', letterSpacing: '-0.01em' }}>Recent Posts</div>
              <button onClick={() => openModal('post')} style={{ width: 22, height: 22, borderRadius: 6, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(56,189,248,0.08)', border: '1px solid rgba(56,189,248,0.15)', color: '#38bdf8', cursor: 'pointer' }}>
...
function TabCoachMembers({ allMemberships, checkIns, ci30, avatarMap, openModal, now }) {
  ...
  const ci30Set = new Set(ci30.map(c => c.user_id));

  const enriched = allMemberships.map(m => {
    const last    = memberLastCI[m.user_id];
    const daysAgo = last ? Math.floor((now - new Date(last)) / 86400000) : null;
    const visits  = ci30.filter(c => c.user_id === m.user_id).length;
    const status  = !last ? 'inactive' : daysAgo >= 14 ? 'at_risk' : daysAgo <= 3 ? 'active' : 'regular';
    return { ...m, last, daysAgo, visits, status };
  });
  ...
  const counts = { all: enriched.length, active: enriched.filter(m => m.status === 'active').length, at_risk: enriched.filter(m => m.status === 'at_risk').length, inactive: enriched.filter(m => m.status === 'inactive').length };
  const statusCfg = { active: { color: '#34d399', label: 'Active' }, regular: { color: '#38bdf8', label: 'Regular' }, at_risk: { color: '#f87171', label: 'At Risk' }, inactive: { color: '#64748b', label: 'Inactive' } };

  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      {/* Header stats */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12 }}>
        <CoachKpiCard icon={Users}       label="Total Members"   value={allMemberships.length} sub="in your gym"        accentColor="#0ea5e9"/>
        <CoachKpiCard icon={Activity}    label="Active This Week" value={new Set(checkIns.filter(c => isWithinInterval(new Date(c.check_in_date), { start: subDays(now,7), end: now })).map(c => c.user_id)).size} sub="checked in"  accentColor="#10b981"/>
        <CoachKpiCard icon={AlertCircle} label="At Risk"          value={enriched.filter(m => m.status === 'at_risk').length}  sub="14+ days inactive" accentColor="#ef4444" subColor="#f87171"/>
        <CoachKpiCard icon={UserCheck}   label="Never Visited"    value={enriched.filter(m => m.status === 'inactive').length} sub="no check-ins yet"  accentColor="#64748b"/>
      </div>
      ...
      <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
        <input
          ...
          style={{ flex: 1, padding: '9px 14px', borderRadius: 10, background: '#0c1a2e', border: '1px solid rgba(255,255,255,0.07)', color: '#f0f4f8', fontSize: 12, outline: 'none' }}
        />
        <div style={{ display: 'flex', gap: 3, padding: '3px', background: 'rgba(255,255,255,0.02)', borderRadius: 10, border: '1px solid rgba(255,255,255,0.06)', flexShrink: 0 }}>
          {[{ id: 'all', label: 'All' }, { id: 'active', label: 'Active' }, { id: 'at_risk', label: 'At Risk' }, { id: 'inactive', label: 'Inactive' }].map(f => (
            <button key={f.id} onClick={() => setFilter(f.id)} style={{ padding: '5px 10px', borderRadius: 8, border: filter === f.id ? '1px solid rgba(255,255,255,0.08)' : '1px solid transparent', background: filter === f.id ? '#0c1a2e' : 'transparent', color: filter === f.id ? '#f0f4f8' : '#3a5070', fontSize: 11, fontWeight: filter === f.id ? 700 : 500, cursor: 'pointer', whiteSpace: 'nowrap' }}>
              {f.label} {f.id !== 'all' && counts[f.id] > 0 && <span style={{ marginLeft: 4, fontSize: 9, fontWeight: 800, color: filter === f.id ? '#38bdf8' : '#3a5070' }}>{counts[f.id]}</span>}
            </button>
          ))}
        </div>
      </div>
      ...
      <CoachCard style={{ overflow: 'hidden' }}>
        {filtered.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '32px 0', color: '#3a5070' }}>
          ...
          const sc  = statusCfg[m.status] || statusCfg.regular;
          const pct = Math.min(100, (m.visits / 20) * 100);
          return (
            <div key={m.user_id || i} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px', borderBottom: i < filtered.length - 1 ? '1px solid rgba(255,255,255,0.04)' : 'none', transition: 'background 0.12s', cursor: 'default' }}
              onMouseEnter={e => e.currentTarget.style.background = 'rgba(255,255,255,0.025)'}
              onMouseLeave={e => e.currentTarget.style.background = 'transparent'}>
              <div style={{ width: 36, height: 36, borderRadius: '50%', background: avatarMap[m.user_id] ? 'transparent' : 'linear-gradient(135deg,#0ea5e9,#8b5cf6)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: 800, color: '#fff', flexShrink: 0, overflow: 'hidden', position: 'relative' }}>
                {avatarMap[m.user_id] ? <img src={avatarMap[m.user_id]} style={{ width: '100%', height: '100%', objectFit: 'cover' }}/> : (m.user_name || '?').charAt(0).toUpperCase()}
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 3 }}>
                  ...
                  <span style={{ fontSize: 9, fontWeight: 800, color: sc.color, background: `${sc.color}15`, borderRadius: 4, padding: '1px 5px', flexShrink: 0 }}>{sc.label}</span>
                </div>
              ...
              <div style={{ flexShrink: 0, width: 64 }}>
                <div style={{ fontSize: 11, fontWeight: 700, color: '#f0f4f8', textAlign: 'right', marginBottom: 3 }}>{m.visits} <span style={{ fontSize: 9, color: '#3a5070', fontWeight: 500 }}>/ mo</span></div>
                <div style={{ height: 3, borderRadius: 99, background: 'rgba(255,255,255,0.05)', overflow: 'hidden' }}>
                  <div style={{ height: '100%', width: `${pct}%`, background: `linear-gradient(90deg,${sc.color},${sc.color}99)`, borderRadius: 99 }}/>
                </div>
              </div>
              <button onClick={() => openModal('post')} style={{ width: 28, height: 28, borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(56,189,248,0.06)', border: '1px solid rgba(56,189,248,0.12)', color: '#38bdf8', cursor: 'pointer', flexShrink: 0 }}>
...
export default function GymOwnerDashboard() {
  ...
  // Classes this coach teaches (match by name/email)
  ...
  // ── Tab content — coach gets different views ──────────────────────────────
  const tabContent = {
    overview: isCoach
      ? <TabCoachOverview myClasses={myClasses} checkIns={checkIns} allMemberships={allMemberships} avatarMap={avatarMap} openModal={openModal} now={now} selectedGym={selectedGym} posts={posts} events={events} challenges={challenges} polls={polls}/>
      : <TabOverview todayCI={todayCI} yesterdayCI={yesterdayCI} todayVsYest={todayVsYest} activeThisWeek={activeThisWeek} totalMembers={totalMembers} retentionRate={retentionRate} newSignUps={newSignUps} monthChangePct={monthChangePct} ciPrev30={ciPrev30} atRisk={atRisk} sparkData={sparkData7} monthGrowthData={monthGrowthData} cancelledEst={cancelledEst} peakLabel={peakLabel} peakEndLabel={peakEndLabel} peakEntry={peakEntry} satVsAvg={satVsAvg} monthCiPer={monthCiPer} checkIns={checkIns} allMemberships={allMemberships} challenges={challenges} posts={posts} polls={polls} classes={classes} coaches={coaches} streaks={streaks} recentActivity={recentActivity} chartDays={chartDays} chartRange={chartRange} setChartRange={setChartRange} avatarMap={avatarMap} priorities={priorities} selectedGym={selectedGym} now={now} openModal={openModal} setTab={setTab} Spark={Spark} Delta={Delta}/>,
    members:  isCoach
      ? <TabCoachMembers allMemberships={allMemberships} checkIns={checkIns} ci30={ci30} avatarMap={avatarMap} openModal={openModal} now={now}/>
