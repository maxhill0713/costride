  ...
  const tabPanels = NAV.map((item) => {
    ...
    if (item.id === 'overview' && !isCoach) {
      ...
      content = isCoach ?
        ...
        <TabContentComponent events={events} challenges={challenges} polls={polls} posts={posts} classes={classes} checkIns={checkIns} ci30={ci30} avatarMap={avatarMapFull} nameMap={memberNameMap} currentUser={currentUser} gym={selectedGym} leaderboardView={leaderboardView} setLeaderboardView={setLeaderboardView} openModal={openModal} now={now} onDeletePost={(id, reason) => deletePostM.mutate({ id, reason })} onUpdatePost={() => inv('posts')} onDeleteEvent={(id) => deleteEventM.mutate(id)} onUpdateEvent={() => inv('events')} onDeleteChallenge={(id) => deleteChallengeM.mutate(id)} onDeleteClass={(id) => deleteClassM.mutate(id)} onDeletePoll={(id) => deletePollM.mutate(id)} memberCount={totalMembers} />;
