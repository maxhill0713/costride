          ...
          <div className="space-y-3">
            {atRisk > 0 ? (
              <div className="p-3.5 rounded-xl" style={{background:'rgba(249,115,22,0.08)',border:'1px solid rgba(249,115,22,0.2)'}}>
                <div className="flex gap-3"><AlertTriangle className="w-4 h-4 text-orange-400 flex-shrink-0 mt-0.5"/>
                  <div><p className="text-sm font-bold text-white">{atRisk} members at risk</p><p className="text-xs mt-0.5" style={{color:'#6b87b8'}}>No check-in for 14+ days</p><button onClick={()=>setTab('members')} className="mt-2 text-xs font-bold text-orange-400 hover:text-orange-300">View in Members →</button></div>
                </div>
              </div>
            ) : (
            ...
            {!challenges.some(c=>c.status==='active') && (
              <div className="p-3.5 rounded-xl" style={{background:'rgba(59,130,246,0.08)',border:'1px solid rgba(59,130,246,0.2)'}}>
                <div className="flex gap-3"><Trophy className="w-4 h-4 text-blue-400 flex-shrink-0 mt-0.5"/>
                  <div><p className="text-sm font-bold text-white">No active challenges</p><p className="text-xs mt-0.5" style={{color:'#6b87b8'}}>Challenges boost retention</p><button onClick={()=>openModal('challenge')} className="mt-2 text-xs font-bold text-blue-400 hover:text-blue-300">Create one →</button></div>
                </div>
              </div>
            )}
            {polls.length===0 && (
              <div className="p-3.5 rounded-xl" style={{background:'rgba(139,92,246,0.08)',border:'1px solid rgba(139,92,246,0.2)'}}>
                <div className="flex gap-3"><BarChart2 className="w-4 h-4 text-purple-400 flex-shrink-0 mt-0.5"/>
                  <div><p className="text-sm font-bold text-white">No active polls</p><p className="text-xs mt-0.5" style={{color:'#6b87b8'}}>Engage members with a poll</p><button onClick={()=>openModal('poll')} className="mt-2 text-xs font-bold text-purple-400 hover:text-purple-300">Create one →</button></div>
                </div>
              </div>
            )}
          </div>
