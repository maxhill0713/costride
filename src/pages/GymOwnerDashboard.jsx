...
export default function GymOwnerDashboard() {
  ...
  const TabContent = () => {
    ...
    // Leaderboard from check-ins last 30 days
    ...
    // Full leaderboard for LeaderboardSection component
    ...
    // Placeholder thumbnail gradient for posts without images
    const postGradients = [
    ...
    return (
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 268px', gap: 16, alignItems: 'start' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          {/* Gradient hero action cards */}
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12 }}>
            {[
              {
                icon: MessageSquarePlus,
                label: 'New Post',
                sub: 'Share with members',
              ...
              },
              {
                ...
                grad: 'linear-gradient(135deg,#3a1010 0%,#5a1a1a 50%,#dc2626 100%)',
              ...
              },
              {
                icon: BarChart2,
                label: 'New Poll',
                sub: `${polls.length} active`,
                grad: 'linear-gradient(135deg,#1e0a3a 0%,#2d1060 50%,#7c3aed 100%)',
          ...
          <Card style={{ padding: 20 }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
              <div>
                ...
                <div style={{ fontSize: 11, color: 'var(--text3)', marginTop: 2 }}>Recent Posts</div>
              </div>
              <button onClick={() => openModal('post')}
                style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '7px 13px', borderRadius: 9, background: 'rgba(14,165,233,0.12)', color: '#38bdf8', border: '1px solid rgba(14,165,233,0.25)', fontSize: 11, fontWeight: 700, cursor: 'pointer' }}>
            ...
            {posts.length > 0 ? (
              ...
              <Empty icon={FileText} label="No posts yet — create the first one!"/>
            )}
          </Card>
          ...
          <Card style={{ padding: 20 }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
              ...
              <button onClick={() => openModal('event')}
                style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '7px 13px', borderRadius: 9, background: 'rgba(16,185,129,0.12)', color: '#34d399', border: '1px solid rgba(16,185,129,0.25)', fontSize: 11, fontWeight: 700, cursor: 'pointer' }}>
                <Plus style={{ width: 11, height: 11 }}/> New Post
              </button>
            </div>
            ...
            {upcomingEvents.length > 0 ? (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                {upcomingEvents.slice(0, 4).map((ev, idx) => {
                  ...
                  return (
                    <div key={ev.id}
                      style={{ display: 'flex', gap: 13, padding: '12px', borderRadius: 12, background: 'rgba(16,185,129,0.04)', border: '1px solid rgba(16,185,129,0.12)', cursor: 'pointer', transition: 'background 0.15s' }}
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '3px 8px', borderRadius: 99, background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.09)' }}>
                            <Avatar name={ev.created_by || ev.gym_name || '?'} size={16}/>
                            <span style={{ fontSize: 10, fontWeight: 600, color: 'var(--text2)' }}>{ev.created_by || ev.gym_name}</span>
                          </div>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '3px 8px', borderRadius: 99, background: 'rgba(139,92,246,0.1)', border: '1px solid rgba(139,92,246,0.2)' }}>
                            <MessageCircle style={{ width: 10, height: 10, color: '#a78bfa' }}/>
                            <span style={{ fontSize: 10, fontWeight: 700, color: '#a78bfa' }}>{ev.attendees || 0} comments</span>
                          </div>
                        </div>
                      </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <Card style={{ padding: 0, overflow: 'hidden' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 16px 10px' }}>
              ...
              <button onClick={() => openModal('challenge')}
                style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '4px 10px', borderRadius: 7, background: 'rgba(245,158,11,0.12)', color: '#fbbf24', border: '1px solid rgba(245,158,11,0.25)', fontSize: 10, fontWeight: 700, cursor: 'pointer' }}>
                + Active
              </button>
            </div>
            {activeChallenges.length > 0 ? (
              ...
              <div style={{ padding: '0 16px 16px' }}>
                ...
                <button onClick={() => openModal('challenge')}
                  style={{ width: '100%', marginTop: 8, padding: '9px', borderRadius: 9, background: 'rgba(245,158,11,0.1)', color: '#fbbf24', border: '1px solid rgba(245,158,11,0.2)', fontSize: 11, fontWeight: 700, cursor: 'pointer' }}>
                  + Create Challenge
                </button>
              </div>
            )}
          ...
          <Card style={{ padding: 16 }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
              ...
              <button onClick={() => openModal('poll')}
                style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '4px 10px', borderRadius: 7, background: 'rgba(139,92,246,0.12)', color: '#a78bfa', border: '1px solid rgba(139,92,246,0.25)', fontSize: 10, fontWeight: 700, cursor: 'pointer' }}>
                + New
              </button>
            </div>
            {polls.length > 0 ? (
                ...
                {milestones.map((m, i) => (
                  <div key={m.name} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '9px 0', borderBottom: i < milestones.length - 1 ? '1px solid rgba(255,255,255,0.05)' : 'none' }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ fontSize: 10, color: 'var(--text3)', marginTop: 1 }}>
                        {m.toNext === 1 ? '1 visit to go!' : `${m.toNext} visits to go`}
                      </div>
                    </div>
