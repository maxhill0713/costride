...
const SETTINGS_LIST = [
  {
    ...
    icon: '🔐',
    sub: 'Password, email & security',
    iconBg: 'linear-gradient(135deg, rgba(239,68,68,0.2), rgba(185,28,28,0.3))',
    keywords: ['account', 'password', 'email', 'security', 'change password'],
  },
  {
    name: 'Profile',
    page: 'ProfileSettings',
    icon: '👤',
    sub: 'Avatar, banner & bio',
    iconBg: 'linear-gradient(135deg, rgba(96,165,250,0.2), rgba(37,99,235,0.3))',
    keywords: ['profile', 'avatar', 'banner', 'bio', 'name', 'picture', 'photo'],
  },
  {
    name: 'Privacy',
    page: 'PrivacySettings',
    icon: '🔒',
    sub: 'Visibility & profile control',
    iconBg: 'linear-gradient(135deg, rgba(52,211,153,0.2), rgba(5,150,105,0.3))',
    keywords: ['privacy', 'public', 'profile', 'private', 'visibility'],
  },
  {
    name: 'Notifications',
    page: 'NotificationSettings',
    icon: '🔔',
    sub: 'Alerts, push & email',
    iconBg: 'linear-gradient(135deg, rgba(251,191,36,0.2), rgba(180,83,9,0.3))',
    keywords: ['notification', 'alert', 'push', 'email', 'message'],
  },
  {
    name: 'Appearance',
    page: 'AppearanceSettings',
    icon: '🎨',
    sub: 'Theme, units & language',
    iconBg: 'linear-gradient(135deg, rgba(167,139,250,0.2), rgba(109,40,217,0.3))',
    keywords: ['appearance', 'dark mode', 'theme', 'unit', 'language', 'settings'],
  },
  {
    name: 'Subscriptions',
    page: 'SubscriptionSettings',
    icon: '💳',
    sub: 'Plan, billing & payment',
    iconBg: 'linear-gradient(135deg, rgba(251,113,133,0.2), rgba(190,18,60,0.3))',
    keywords: ['subscription', 'plan', 'billing', 'payment'],
    badge: { label: 'PRO', style: { background: 'linear-gradient(135deg, #f59e0b, #d97706)', color: '#fff' } },
  },
  {
    name: 'Help & Support',
    page: 'HelpSupport',
    icon: '❓',
    sub: 'FAQ, contact & feedback',
    iconBg: 'linear-gradient(135deg, rgba(148,163,184,0.15), rgba(71,85,105,0.3))',
    keywords: ['help', 'support', 'faq', 'question', 'contact'],
  },
];
  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ fontSize: 12, fontWeight: 500, color: '#475569', marginTop: 1 }}>
          {setting.sub}
        </div>
      </div>
...
export default function Settings() {
  ...
  const displayedSettings = useMemo(() => {
    if (!searchQuery.trim()) return SETTINGS_LIST;
    const q = searchQuery.toLowerCase();
    return SETTINGS_LIST.filter(
          ...
          <>
            {GROUPS.map((group) => {
              const rows = group.pages
                .map((page) => SETTINGS_LIST.find((s) => s.page === page))
                .filter(Boolean);
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginTop: 8 }}>
              ...
              <AlertDialog>
                ...
                <AlertDialogContent style={{
                  ...
                  <AlertDialogHeader>
                    <AlertDialogTitle style={{ color: '#fff', fontWeight: 900 }}>
                      ⚠️ Delete Account?
                    </AlertDialogTitle>
                    <AlertDialogDescription style={{ color: '#94a3b8' }}>
                      ...
                      {currentUser.account_type === 'gym_owner' ? ', and all gyms you own' : ''}. This cannot be undone.
