...
export default function Gyms() {
  ...
  const { data: gymMemberships = [] } = useQuery({ queryKey: ['gymMemberships', currentUser?.id], queryFn: () => base44.entities.GymMembership.filter({ user_id: currentUser?.id, status: 'active' }), enabled: !!currentUser?.id, staleTime: 5 * 60 * 1000, gcTime: 15 * 60 * 1000, placeholderData: (prev) => prev });
  const { data: gyms = [] } = useQuery({ queryKey: ['gyms'], queryFn: () => base44.entities.Gym.filter({ status: 'approved' }, 'name', 100), staleTime: 10 * 60 * 1000, gcTime: 30 * 60 * 1000, placeholderData: (prev) => prev });

  const recentlyViewedGymIds = React.useMemo(() => {
    if (!currentUser?.id) return [];
    try { return JSON.parse(localStorage.getItem(`recentlyViewedGyms_${currentUser.id}`) || '[]').slice(0, 3); }
    catch { return []; }
  }, [currentUser?.id]);
  ...
  const { data: userGymsData = [] } = useQuery({
    ...
    queryFn: () => { if (memberGymIds.length === 0) return []; return base44.entities.Gym.filter({ id: { $in: memberGymIds } }); },
    enabled: !!currentUser && gymMemberships.length > 0, staleTime: 10 * 60 * 1000, gcTime: 30 * 60 * 1000, placeholderData: (prev) => prev
  });
  ...
  const updateGymImageMutation = useMutation({
    ...
    onMutate: async ({ gymId, image_url }) => {
      ...
      queryClient.setQueryData(['gyms'], (old = []) => old.map((gym) => gym.id === gymId ? { ...gym, image_url } : gym));
      return { previous };
    },
    onError: (err, variables, context) => { if (context?.previous) { queryClient.setQueryData(['gyms'], context.previous); } },
  ...
  const updatePrimaryGymMutation = useMutation({
    ...
    onError: (err, gymId, context) => { queryClient.setQueryData(['currentUser'], context.previous); },
  ...
  const leaveGymMutation = useMutation({
    ...
    onError: (err, gymId, context) => { queryClient.setQueryData(['gymMemberships', currentUser?.id], context.previous); },
  ...
  const searchPlaces = async (query) => {
    ...
    } catch (error) { console.error('Places search failed:', error); setPlacesResults([]); }
  ...
  const handleCreateGym = async () => {
    ...
    if (gymMemberships.length >= 3 && !isOwner) { alert('You can only be a member of up to 3 gyms. Please leave a gym before joining a new one.'); return; }
    if (!isOwner) {
      const userCreatedGhostGyms = gyms.filter((g) => g.created_by === currentUser?.email && !g.admin_id && !g.owner_email);
      if (userCreatedGhostGyms.length >= 3) { alert('You have reached the limit of 3 ghost gyms you can create.'); return; }
    }
  ...
  // ── Shared gym card inner layout ──
  const GymCardInner = ({ gym, isMember = false }) => (
    <div
      className="relative rounded-2xl overflow-hidden transition-all duration-300 group"
      style={{ background: 'linear-gradient(135deg, rgba(30,35,60,0.72) 0%, rgba(8,10,20,0.88) 100%)', border: '1px solid rgba(255,255,255,0.07)', backdropFilter: 'blur(16px)', WebkitBackdropFilter: 'blur(16px)', boxShadow: '0 4px 24px rgba(0,0,0,0.4)' }}>
      ...
      <div className="relative w-full h-48 bg-gradient-to-br from-slate-700 to-slate-800 overflow-hidden">
        ...
        <Link to={createPageUrl('GymCommunity') + '?id=' + gym.id} onMouseEnter={() => prefetchGymData(gym.id)} onMouseDown={() => prefetchGymData(gym.id)} className="absolute inset-0 flex items-center justify-center">
          <Button className="inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md font-bold focus-visible:outline-none disabled:pointer-events-none disabled:opacity-50 py-2 bg-gradient-to-b from-blue-500 via-blue-600 to-blue-700 backdrop-blur-md text-white border border-transparent text-xs h-8 px-3 shadow-[0_3px_0_0_#1a3fa8,0_8px_20px_rgba(0,0,100,0.5),inset_0_1px_0_rgba(255,255,255,0.15)] active:shadow-none active:translate-y-[3px] active:scale-95 transition-all duration-100 transform-gpu">
        ...
        <div className="absolute top-3 right-3 flex gap-3">
          ...
          {isMember ? (
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="bg-slate-800 border border-slate-700/50 rounded-lg shadow-[0_3px_0_0_#1e293b,0_8px_20px_rgba(0,0,0,0.3)] active:shadow-none transform-gpu">
        ...
        >
          <div className="max-w-6xl mx-auto">
            ...
            <div className="relative flex items-end justify-center h-[2.9rem] pb-1">
              ...
              {/* Right-side action buttons — pinned to bottom too */}
              <div className="absolute right-0 bottom-1 flex items-center">
                <TabsContent value="my-gyms" className="mt-0 p-0 m-0">
                  {userGyms.length > 0 &&
                    <Button onClick={() => setShowPrimaryGymModal(true)} className="inline-flex items-center justify-center whitespace-nowrap font-bold transition-all duration-100 focus-visible:outline-none disabled:pointer-events-none disabled:opacity-50 bg-gradient-to-b from-purple-400 via-purple-500 to-purple-600 text-white border border-transparent rounded-md text-[10px] h-6 px-1.5 shadow-[0_2px_0_0_#5b21b6,inset_0_1px_0_rgba(255,255,255,0.2)] active:shadow-none active:translate-y-[2px] active:scale-95 transform-gpu">
                      Set Home
                    </Button>
                  }
                </TabsContent>
                <TabsContent value="explore" className="mt-0 p-0 m-0">
                  <Button onClick={() => setShowJoinWithCode(true)} className="inline-flex items-center justify-center whitespace-nowrap font-bold transition-all duration-100 focus-visible:outline-none disabled:pointer-events-none disabled:opacity-50 bg-gradient-to-b from-blue-500 via-blue-600 to-blue-700 text-white border border-transparent rounded-md text-[10px] h-6 px-1.5 shadow-[0_2px_0_0_#1a3fa8,inset_0_1px_0_rgba(255,255,255,0.15)] active:shadow-none active:translate-y-[2px] active:scale-95 transform-gpu">
                    Gym Key
                  </Button>
                </TabsContent>
              </div>
              ...
              <div className="flex items-center gap-2">
                <div className="relative flex-1">
                  ...
                  <Input
                    ...
                    style={{ fontSize: '16px' }}
                    className="flex w-full px-3 py-1 text-base shadow-sm h-9 pl-10 pr-10 bg-white/10 border border-white/20 hover:border-white/40 focus-visible:outline-none focus-visible:border-blue-400 focus-visible:bg-white/15 text-white placeholder:text-slate-300 rounded-xl transition-all duration-200" />
                    ...
                    {[...Array(3)].map((_, i) =>
                      <div key={i} className="rounded-xl bg-slate-700/50 border border-slate-600/40 overflow-hidden flex items-stretch gap-0">
                        ...
                        <div className="flex-1 p-3 flex flex-col justify-center gap-2">
                          <div className="h-4 bg-slate-600/60 rounded w-3/4" /><div className="h-3 bg-slate-600/40 rounded w-1/2" /><div className="h-3 bg-slate-600/30 rounded w-16" />
                        </div>
                      </div>
                    )}
                  </div>
        ...
        <DialogContent className="fixed left-[50%] top-[50%] z-50 grid w-full translate-x-[-50%] translate-y-[-50%] gap-4 p-6 max-w-lg max-h-[80vh] overflow-y-auto [&>button]:hidden bg-slate-800/30 backdrop-blur-md border border-slate-700/20 rounded-3xl shadow-2xl shadow-black/20 text-white">
          ...
          <div className="space-y-3">
            {equipmentGym?.equipment && equipmentGym.equipment.length > 0
              ? <div className="grid grid-cols-2 gap-2">{equipmentGym.equipment.map((item, idx) => <div key={idx} className="flex items-center gap-2 p-2 bg-slate-800/60 rounded-lg border border-slate-700/50"><div className="w-1.5 h-1.5 rounded-full bg-blue-400 flex-shrink-0" /><span className="text-slate-200 text-sm">{item}</span></div>)}</div>
        ...
        <DialogContent className="fixed left-[50%] top-[50%] z-50 grid w-full translate-x-[-50%] translate-y-[-50%] gap-4 p-6 max-w-4xl max-h-[110vh] overflow-y-auto [&>button]:hidden bg-slate-800/30 backdrop-blur-md border border-slate-700/20 rounded-3xl shadow-2xl shadow-black/20 text-white">
          <div>
            {galleryGym?.gallery && galleryGym.gallery.length > 0
              ? <div className="grid grid-cols-2 md:grid-cols-3 gap-3">{galleryGym.gallery.map((imageUrl, idx) => <div key={idx} className="aspect-square rounded-lg overflow-hidden bg-slate-700/50 border border-slate-600/50"><img src={imageUrl} alt={`${galleryGym.name} photo ${idx + 1}`} className="w-full h-full object-cover hover:scale-110 transition-transform duration-300" loading="lazy" /></div>)}</div>
        ...
        <DialogContent data-primary-gym-dialog className="fixed left-[50%] top-[50%] z-50 grid w-full translate-x-[-50%] translate-y-[-50%] gap-4 p-6 max-w-lg max-h-[80vh] overflow-y-auto [&>button]:hidden bg-slate-800/30 backdrop-blur-md border border-slate-700/20 rounded-3xl shadow-2xl shadow-black/20 text-white">
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-3 pg-item-in" style={{ animationDelay: `${140 + userGyms.length * 55}ms` }}>
              <Button onClick={() => { setShowPrimaryGymModal(false); setSelectedPrimaryGym(null); }} className="inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-lg text-sm font-bold transition-all duration-100 disabled:pointer-events-none disabled:opacity-50 py-2 bg-gradient-to-b from-slate-600 via-slate-700 to-slate-800 text-white border border-slate-500/40 h-9 px-4 flex-1 shadow-[0_3px_0_0_#1e293b,inset_0_1px_0_rgba(255,255,255,0.12)] active:shadow-none active:translate-y-[3px] active:scale-95 transform-gpu">Cancel</Button>
              <Button onClick={() => { if (selectedPrimaryGym) { updatePrimaryGymMutation.mutate(selectedPrimaryGym); } else { setShowPrimaryGymModal(false); setSelectedPrimaryGym(null); } }} disabled={updatePrimaryGymMutation.isPending} className="inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-lg text-sm font-bold transition-all duration-100 disabled:pointer-events-none disabled:opacity-50 py-2 bg-gradient-to-b from-purple-400 via-purple-500 to-purple-600 text-white border border-transparent h-9 px-4 flex-1 shadow-[0_3px_0_0_#5b21b6,inset_0_1px_0_rgba(255,255,255,0.2)] active:shadow-none active:translate-y-[3px] active:scale-95 transform-gpu">
